...
**🌐 Язык**: [English](TEMPLATE_REFERENCE.md) | Русский
...
### 4.6 `group_templates` — во что материализуется Направление (SPEC 104)

Форма, которую принимает Направление, описана **шаблоном**, а не зашита в
код: лаунчер и LxBox читают одно описание.

```jsonc
"group_templates": {
  "magic_nodes": {                                    // служебные опции Направления
...
- **`magic_nodes`** — теги служебных опций. `source: "preset"` берёт тег как
  есть; `source: "generate"` строит его из `tpl`, подставляя вместо
  `{parent_tag}` тег самого Направления. Теги direct/block читаются отсюда, а
  не предполагаются: они не универсальны, и зашить их значило бы сломать
  чужой шаблон. Тег `block` лаунчер использует ещё и для запасного состава
  пустого Направления.
- **`auto.options`** — умолчания парной группы urltest; уезжают в конфиг как
  есть, **включая ссылки `@var`**: подстановка — задача движка шаблонов, и
  делать её здесь значило бы завести её вторую реализацию. Поля `auto` самого
  Направления перекрывают эти умолчания.
- **`channel`** — имя ключа историческое (в первой редакции SPEC 104 модель
  называлась «каналами»). Язык шаблона общий с LxBox, и переименование ключа
  сломало бы уже существующие шаблоны без всякой выгоды.

Шаблон без этой секции работает: Направления соберутся из того, что задал
пользователь, просто без шаблонных умолчаний.

Только для мобильного приложения: `default_channels` — телефон засевает из
него стартовый набор; лаунчер засевается из `parser_config.outbounds`.
...
> **Паритет с мобильным (SPEC 103):** `#if` реализован с обеих сторон — в
> лаунчере (`core/template/substitute.go`) и в LxBox
> (`app/lib/services/builder/if_engine.dart`); нормативное описание общего языка
> живёт в `contract/docs/TEMPLATE_LANG.md`. При расхождении этого справочника с
> контрактом прав контракт. Остальные `#*`-конструкции (потенциальные
> `#for_each` / `#include`) остаются desktop-only, пока не появятся в LxBox.

**Именованные условия.** Ключ условия может нести любой суффикс: `"#if"`,
`"#if1"`, `"#if 2"`, `"#if tun-only"` равнозначны. Это нужно потому, что JSON
оставляет только последний из двух одинаковых ключей — второй обычный `"#if"` на
том же объекте молча затёр бы первый; суффикс позволяет повесить на один объект
несколько независимых условий и заодно служит понятной подписью. Несколько
`#if…` одного объекта применяются в порядке сортировки ключей (одинаково в обоих
движках). Элементу массива (§9.4) и скалярному `#if` по-прежнему нужен ровно
один такой ключ — с любым суффиксом.

### 9.1 Naming discipline — `#` vs `@` vs данные

**Правило языка одной фразой (SPEC 107, D-073):**

> `#` — ключевое слово движка. `@` — ссылка на переменную. Всё остальное — данные.

| Префикс | Что это | Примеры |
|---------|---------|---------|
| `#` | ключевое слово движка: конструкции, операторы условия, ветки | `#if`, `#enable`, `#and`, `#or`, `#not`, `#value`, `#else`, `#in`, `#notIn`, `#matches`, `#notEmpty`, `#isEmpty`, `#on_change`, `#set` |
| `@` | ссылка на переменную из `vars[]` либо runtime-global | `@tun`, `@tun_mtu`, `@runtime.platform` |
| без префикса | данные — поля sing-box и значения | `outbound`, `rule_set`, `server`, `options[].value` |

Первый символ ключа самодостаточен: разборщику не нужен контекст, а автору
шаблона — помнить, внутри какой конструкции он находится.

**Легаси-формы читаются бессрочно.** До SPEC 107 правило выполнялось
наполовину: `#not`/`#in`/`#matches` были помечены, а `and`/`or`/`value`/`else`/
`on_change`/`set` — нет, хотя интерпретирует их тот же движок. Непомеченные
формы остаются валидными, существующие шаблоны не ломаются; смешанная запись
(канон снаружи, легаси внутри) тоже валидна.

Forward compatibility: неизвестный ключ, начинающийся с `#`, движок логирует
и удаляет (graceful degradation) — новые конструкции можно добавлять без
поломки старых лаунчеров. Побочный эффект помеченного канона: опечатка вида
`#adn` теперь ловится этим же механизмом.

### 9.2 Форма

```jsonc
"#if": {
  "#or":    [<cond>, <cond>, ...],   // взаимоисключающе с "#and"
  "#and":   [<cond>, <cond>, ...],   // взаимоисключающе с "#or"
  "#value": <any JSON>,               // обязателен, then-ветка
  "#else":  <any JSON>                // опционально, else-ветка
}
```

Элемент списка `#and`/`#or` — предикат **или вложенный cond-obj**; глубина не
ограничена (SPEC 107, снят прежний запрет):
...
`#not` отрицает любое условие, включая `#and`/`#or`, а не только предикат.

Правила (валидация на load):
* Ровно один из `#and` / `#or`. Нет / оба → ошибка загрузки.
* `#value` обязателен (не nil).
* `#else` опционален; null в `#value`/`#else` → ошибка в map-spread (нечего
  merge), допустимо в array-element.
* Невалидная форма в толерантном рантайме → **false** + warning (fail-closed):
  опечатка в условии не должна молча включить узел.

Легаси-написание тех же ключей без `#` (`and`, `or`, `value`, `else`)
поддерживается бессрочно.

### 9.2.1 `#enable` — гейт существования узла

Плоский ключ среди полей узла: не подставляет значение, а решает, существует
ли узел вообще.
...
* Значение — любое условие (§9.2), включая сахар-список: `["@a", "@b"]` ≡
  `{"#and": ["@a", "@b"]}`.
* false → узел выпадает: ключ исчезает из объекта, элемент — из массива.
  Внутренности при этом **не вычисляются вовсе**.
* Вычисляется **первым**, до `#if` того же узла.
* Носители: объекты дерева `config`, фрагменты пресета (`rule_set[]`,
  `rules[]`, `dns_servers[]`, `dns_rule(s)`, `outbounds[]`), объявления
  переменных (гасит строку Settings), `params[]` (блок не применяется).
* Легаси-эквиваленты: `if` (≡ `#and`-список), `if_or` (≡ `#or`-список),
  в LxBox `enabled: "@var"` — читаются бессрочно.

Отличие от `#if`: `#if` — подстановка с ветками `#value`/`#else`;
`#enable` — булев гейт без веток.
