...
**🌐 Язык**: [English](WIZARD_STATE.md) | Русский
...
> - узлы контейнера хранятся явно в `nodes[]`, у каждого свои `tag`, `enabled`,
>   `origin` и готовое к эмиссии `body`. Тела подписок на сборке больше не
>   разбираются, кэш сырых ответов (`bin/subscriptions/*.raw`) упразднён;
> - отметка «выключен» живёт на самом узле (`enabled`), а не отдельной картой
>   `disabled_nodes` из хэш-ключей; ссылки (`detour`, хопы цепочек, члены групп) —
>   объекты `NodeLink` вида `{folder_id, tag}`;
> - у подписки своя идентификация запроса: `user_agent`, `hwid`, `send_hwid`,
>   `hash_device_model` и `relays_in_directions`.
>
> Файлы v6 мигрируют сами при первом запуске; прежний файл остаётся рядом
> как `state.json.v6.bak`.
...
```jsonc
{
  ...
  "connections": {
    ...
    "direction_outbounds": [ ... ],  // SPEC 104: Направления — цели, на которые ссылаются правила
                              // (в старых состояниях лежат под "outbounds"; читается бессрочно, не пишется)
...
| `type` | string | всегда | `subscription` \| `server` \| `chain` (SPEC 110). |
...
**Цепочка хопов (`type: chain`, SPEC 110).** `hops` перечисляет позиции в
порядке ПАКЕТА: первая — ближайший к вам хоп, последняя — та, чей адрес
видит цель. У `detour` порядок обратный («кто через кого»), и перепутать их
значит собрать работающий, но не тот маршрут. Позицией может быть узел,
группа подписки, Направление, служебный тег шаблона или другая цепочка.

Цепочка становится узлом ПОСЛЕ загрузки всех источников: её позиции
ссылаются на теги, окончательные только тогда (префиксы подписок,
уникализация дублей). Для остального лаунчера она обычный узел — попадает в
пул, ловится фильтрами Направлений, переключается в Clash API.

Ядро отвергает конфиг ЦЕЛИКОМ, если цепочка нарушает его условия, поэтому
они проверяются до эмиссии: минимум две позиции, ни одной пустой, без
ссылки на саму себя и без повторов. Позиция, не найденная среди узлов и
Направлений, убирает цепочку **целиком**: маршрут без хопа — это другой
маршрут.

Циклы исключены двумя разными способами: цепочка ссылается только на
цепочку, объявленную ВЫШЕ по списку источников (ссылки вперёд отвергаются),
а Направление не берёт в состав цепочку, которая через это же Направление
проходит — транзитивно.

`strip` снимает у звеньев односторонние приёмы обхода DPI; каталог закрыт
(`tls.fragment`, `multiplex.padding`, `xhttp.padding`, `tls.utls`),
неизвестный ключ — ошибка старта. `tls.utls` по умолчанию НЕ снимается и на
узлах с `reality` сниматься не должен: `sing-box check` этого не ловит —
конфиг проходит проверку и падает при запуске.

Ядро, собранное без `with_lx_chain`, типа не знает. Лаунчер проверяет теги
сборки заранее: неподдержанная цепочка не становится узлом вовсе — её
просто нет в пуле, как у выключенного источника, — а причина попадает в лог
и в строку источника.
...
| `vars` | `map[string]string` (omitempty) | Только для `kind=template` (SPEC 129) — значения переменных самого сервера по локальным именам его записи в шаблоне (`outbound`, `dns_ip`, …). Только объявленные имена, только значения, отличные от умолчания шаблона, хранятся подрезанными; нет ключа — «следовать шаблону». |
...
Объявленные записью `vars[]` остаются при сервере (SPEC 129):
`TemplateData.DNSServerVars[tag]` с локальными именами, а тело держит
`@outbound`, `@dns_ip`. Значения живут в записи сервера в состоянии —
`dns.servers[kind=template].vars`, той же, что едет в файле бэкапа и хранится
у мобильного приложения. Правятся в окне самого сервера, не на вкладке
Settings. Плейсхолдер тела обязан быть объявлен сервером или, расширением
лаунчера, переменной шаблона верхнего уровня; иначе шаблон не проходит
валидацию.

До SPEC 129 эти переменные склеивались в переменные шаблона с именем
`dns_<tag>_<var>` и хранились в корневых `vars[]` состояния. Такие имена
по-прежнему читаются: загрузка, сборка, экспорт и импорт переносят их в запись
сервера (при неоднозначности тега с подчёркиванием выигрывает самый длинный),
и следующая запись их уже не несёт. Значение, равное умолчанию шаблона, запись
не хранит.
...
### 3.7 `connections.direction_outbounds[i]` — Направления (SPEC 104)

**Направление** — именованная цель, на которую ссылаются правила. Ссылаться
прямо на узел подписки нельзя: его тег создаётся заново при каждом
обновлении, и правило разваливалось бы само собой. На сборке Направление
материализуется в `selector` и, если включён автовыбор, в парную группу
`urltest` с именем `<tag>-auto`.

| Поле | Тип | Значение |
|------|-----|----------|
| `tag` | string | Идентификатор, на который ссылаются правила. Неизменяем после создания: переименование сломало бы все ссылки. Выдаваемые автоматически имеют вид `vpn-1`; шаблонные и пресетные — произвольные (`proxy-out`, `ru VPN 🇷🇺`). |
| `label` | string | Отображаемое имя. Пусто — показываем тег. Правится свободно: на имя никто не ссылается. |
| `disabled` | bool | Не собирается и не предлагается целью правил. Именно `disabled`, а не `enabled`: нулевое значение bool обязано означать «включено», иначе запись без ключа читалась бы выключенной. |
| `type` | string | `selector` у Направления; `urltest` — у самостоятельных auto-групп шаблона (`auto-proxy-out`) |
| `filters` | object | Отбор узлов на общем языке паттернов (`/re/i`, `!/re/i`). Форма показывает только **тело** и галку инверсии; флаг `i` пишется всегда. |
| `preferredDefault` | object | Тот же язык; первый совпавший узел становится `default` селектора. |
| `addOutbounds` | array | Дополнительные опции: `direct-out`, `block-out` и Направления, стоящие **выше** по списку. Парный `<tag>-auto` — никогда: он опция только внутри своего Направления. |
| `auto` | object \| null | Параметры двойника: `mode` (`least_test` \| `round_robin`), `url`, `interval`, `tolerance`, `idle_timeout`, `interrupt_exist_connections`, плюс `pool` / `pool_tolerance` / `sticky_hash` для round-robin. **null означает, что двойника нет вовсе.** |
| `options`, `comment`, `required`, `ref`, `updates` | | Как раньше (SPEC 057/058): привязка к шаблону и пресетам, стек патчей. |

**Двойник не хранится.** `<tag>-auto` разворачивается на каждой сборке из
`auto`. Держать его в состоянии значило бы завести вторую сущность, которую
пользователь обязан вручную поддерживать в согласии с первой.

**Старый ключ читается бессрочно.** Состояние, записанное до SPEC 104,
хранит Направления под ключом `connections.outbounds`; при загрузке они
переносятся и обратно уже не пишутся. При наличии обоих ключей побеждает
канонический — иначе состояние, потроганное старой версией после новой,
склеило бы два набора с дублирующимися тегами.
...
### 6.5 Удалённые поля (sing-box 1.14)
