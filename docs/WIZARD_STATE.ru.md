...
```jsonc
{
  ...
  "channels": [                          // SPEC 104; секции нет = ещё не засеяно
...
### 3.7 `channels[i]` — каналы роутинга (SPEC 104)

Канал — **именованная цель**, на которую ссылаются правила. Ссылаться прямо на
узел подписки нельзя: теги узлов создаются заново при каждом обновлении, и такое
правило развалилось бы само собой. На сборке канал материализуется в `selector`
и, при включённом автовыборе, в парный `urltest` с тегом `<tag>-auto`.

| Поле | Тип | Смысл |
|------|-----|-------|
| `tag` | string | `vpn-1`…`vpn-10`. **Неизменяем** — на него ссылаются правила, и переименование не должно их ломать. Пользователю не редактируется. |
| `label` | string | Отображаемое имя. Единственное «имя», которое вводит пользователь. |
| `enabled` | bool | Умолчание — **true**. Отсутствие ключа означает true, а не false (см. примечание ниже). |
| `include_direct` | bool | Предлагать `direct-out` опцией внутри селектора. |
| `include_block` | bool | Предлагать блокирующий outbound опцией. |
| `node_filter` | string | Регулярное выражение по **итоговому** тегу узла. Пусто = все узлы. |
| `node_filter_invert` | bool | Оставить узлы, которые **не** совпали. При пустом фильтре не значит ничего. |
| `default_filter` | string | Регулярное выражение; первый совпавший узел становится `default` селектора. |
| `interrupt_exist_connections` | bool | Умолчание — **true**. Рвать живые соединения при переключении канала. |
| `auto` | object \| null | `{url, interval, tolerance, idle_timeout, interrupt_exist_connections}`. **null означает, что автовыбора нет**, и `<tag>-auto` не эмитится вовсе. |

**Отсутствие секции против пустого массива.** Нет ключа `channels` вовсе —
значит «ещё не засеяно», и лаунчер один раз заполняет секцию из
`default_channels` шаблона. **Пустой массив** — это выбор самого пользователя
(«я удалил все каналы»), и повторный seed вернул бы удалённое на следующем
старте. Два состояния различаются намеренно.

**Умолчания при чтении.** `Channel.UnmarshalJSON` восстанавливает `enabled` и
`interrupt_exist_connections` в true, когда ключей нет: нулевое значение bool —
false, и без этого канал, записанный без явного `"enabled"`, молча выключал бы
себя при каждом чтении.

Форма канала и материализация: `core/state/channel_types.go`,
`core/build/channels.go`. Шаблонная часть (`group_templates`,
