...
| `type` | string | `selector` у Направления; `urltest` — у самостоятельных auto-групп шаблона (`auto-proxy-out`); `chain` — у цепочки хопов (SPEC 110). |
...
| `chain` | object \| null | **SPEC 110.** Цепочка хопов вместо селектора: `hops` (позиции в порядке пакета), `idle_timeout`, `strip_evasion`, `strip`, `rewrite`. Присутствие ключа вместе с `type: "chain"` означает, что у записи нет ни состава, ни фильтра, ни автовыбора. |
...
**Цепочка (SPEC 110).** `hops` перечисляет позиции в порядке ПАКЕТА: первая —
ближайший к вам хоп, последняя — та, чей адрес видит цель. У `detour` порядок
обратный («кто через кого»), и перепутать их значит собрать работающий, но не
тот маршрут. Позицией может быть узел, группа подписки, другое Направление или
служебный тег шаблона.

Ядро отвергает конфиг ЦЕЛИКОМ, если цепочка нарушает его условия, поэтому они
проверяются до эмиссии: минимум две позиции, ни одной пустой, без ссылки на
саму себя и без повторов; вложенная цепочка — только первой позицией. Позиция,
не дошедшая до конфига, убирает цепочку **целиком**, а не одну позицию:
маршрут без хопа — это другой маршрут.

`strip` снимает у звеньев односторонние приёмы обхода DPI; каталог закрыт
(`tls.fragment`, `multiplex.padding`, `xhttp.padding`, `tls.utls`), неизвестный
ключ — ошибка старта. `tls.utls` по умолчанию НЕ снимается и на узлах с
`reality` сниматься не должен: отпечаток ClientHello там несущий элемент
протокола.

Ядро, собранное без `with_lx_chain`, типа не знает. Лаунчер проверяет теги
сборки заранее: неподдержанная цепочка не эмитится вовсе, правила на неё
уходят на `route.final`, причина попадает в лог и в окно правки записи.
