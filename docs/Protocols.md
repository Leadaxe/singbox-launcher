...
`reality_fp_not_chrome` (a REALITY node carries an explicit fingerprint outside the
Chrome family — kept as is; Xray ≥ v26.9.8 servers reject it), `packet_encoding_unknown`,
...
- **⚠️ REALITY: an explicit fingerprint is kept, an empty one becomes Chrome (D-119).** Whenever a `tls.reality` block is actually emitted, uTLS is always enabled (the core refuses REALITY without it). A fingerprint the node set explicitly goes into `config.json` as is; an empty `fp` and our own implicit default `random` are written as `chrome`. A REALITY server running Xray ≥ v26.9.8 requires a `key_share X25519MLKEM768` that the Chrome uTLS specs carry, and since core `1.14.1-lx.3` also `firefox` and `safari`; so a node with an explicit fingerprint outside that set (`ios`, `android`, `edge`, `360`, `qq`) gets the `reality_fp_not_chrome` code: if it does not connect, try `chrome`. The fingerprint is not replaced for you: it is the subscription's choice, and the launcher does what the subscription says (D-104, which forced Chrome, was replaced).
- **`key_share` — REALITY key exchange (core ≥ `1.14.1-lx.4`, D-121).** The link parameter `key_share=hybrid|classical` on `vless://` and `anytls://`, next to `pbk`/`sid` — the sing-box key name as is. `hybrid` requires `X25519MLKEM768`, `classical` strips it (needed by REALITY servers on Xray **older** than v26.9.8, which drop a hybrid ClientHello); empty = whatever the uTLS fingerprint carries. Read **only when `pbk` is valid** — on a non-REALITY node the field is meaningless. Any other value is **dropped** with the `reality_key_share_invalid` code (the node survives): the core rejects such a value by refusing the WHOLE config, not the single node. On a core older than `1.14.1-lx.4` the field is not emitted at all — the key is unknown to it, and an unknown key brings the whole config down; the node stays REALITY.
