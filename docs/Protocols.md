...
- **⚠️ REALITY: an explicit fingerprint is kept, an empty one becomes Chrome (D-119).** Whenever a `tls.reality` block is actually emitted, uTLS is always enabled (the core refuses REALITY without it). A fingerprint the node set explicitly goes into `config.json` as is; an empty `fp` and our own implicit default `random` are written as `chrome`. A REALITY server running Xray ≥ v26.9.8 requires a `key_share X25519MLKEM768` that only the Chrome uTLS specs carry, so a node with an explicit fingerprint outside the Chrome family gets the `reality_fp_not_chrome` code: if it does not connect, try `chrome`. The fingerprint is not replaced for you: older servers accept `firefox` without MLKEM, and Chrome's hybrid key makes a ClientHello larger than one TCP segment, which DPI in mobile networks cuts — a provider that set `firefox` may have done so on purpose (D-104, which forced Chrome, was replaced after a field report).
