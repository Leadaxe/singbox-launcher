...
| `node_parser_transport.go` | What the tables do not express as data: REALITY uTLS enforcement at build time (`EnforceRealityFingerprint`). uTLS fingerprint values and WS early data are registry rules executed by the engine. |
...
| `backend_daemon.go` (+ `_dns`, `_traffic`, `_tailscale`) | `DaemonBackend` — lxd engine: gRPC control plane, structured DNS, connection and tailnet streams for the profiler. Daemon platforms: `darwin \|\| (windows && !386)` (SPEC 141). |
...
| `backend_daemon_stub.go` (+ `debugapi_wiring_daemon_stub.go`, `purge_other.go`) | Stubs for Linux and Win7 (`!darwin && (!windows \|\| 386)`) so the rest of the code compiles without the engine (this is what keeps the Win7 build clean). |
| `daemon_manager.go` (+ `_darwin.go`, `_windows.go`) | Daemon lifecycle from the launcher's side: the command strings it hands the user (`--service=install` / `=uninstall [--purge]` / `lxd client add`), pairing, daemon passport (`GET /admin/info`), config preparation, the dialog after a core update. `_darwin.go`: launchd label and plist, bootstrap / kickstart, sudo rendering, Terminal; `_windows.go`: extension-point stubs, the engine closed (SPEC 141). Runs nothing privileged itself. |
| `daemon_service_state.go` (+ `_darwin.go`, `_windows.go`) | **SPEC 136.** Service classifier: reads the launchd plist, checks the root-owned copy of the core and its ownership chain (`/Library` → `PrivilegedHelperTools` → the flat copy file), compares sha256 with the launcher core (cached by dev/inode/size/mtime) and with the running daemon's passport. Verdicts: not installed / unsafe / stale / process stale / ok, and core too old — the install and copy commands are gated on the launcher core version (≥ 1.14.1-lx.12, its release candidates included). Reads only, no sudo. Shared: states, version gate, sha and process verdicts, hash and version caches; `_darwin.go`: `/Library` layout, plist, the uid chain link (`Stat_t`), the cache key, launchd; `_windows.go`: stubs (the service is "not installed"). |
| `classic_privileged.go` (+ `_darwin.go`, `_windows.go`, `_other.go`) | **SPEC 137.** Gate of the classic privileged (TUN) start: the root-owned copy of the core must exist, pass the ownership chain and match the launcher core by sha256 (functions and hash cache of `daemon_service_state.go`); dialog texts in `_darwin.go`; otherwise a dialog with one sudo command (`lxd --service=copy`, or `--service=install` when the daemon service is installed) and Retry; a WARN after a core download when the copy falls behind. Reads only, no sudo. |
