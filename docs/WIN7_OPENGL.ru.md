...
Обновление: с версии 1.4.2 **Win64**-сборка делает ровно это — проверяет OpenGL на старте и предлагает автозагрузку Mesa (см. [RDP_OPENGL.ru.md](RDP_OPENGL.ru.md), issue #105). Для Win7 32-bit остаются ручные шаги выше: современные сборки Mesa требуют Windows 10+, автоустановленная версия там не заведётся. На Win7 лаунчер показывает нативный диалог со ссылкой на эту инструкцию.
