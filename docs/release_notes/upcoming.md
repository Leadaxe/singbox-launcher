...
### Technical / Internal
- Tailscale node state directories now follow the node: renaming a node, moving it into or out of a folder, or changing a folder's tag policy moves the directory instead of orphaning it, so the tailnet device identity (and its burnt one-off auth key) survives; deleting the node or its folder removes the directory, and each build sweeps directories left behind by nodes that no longer exist. A `state_directory` you set yourself in the node body is never touched.
...
- Каталог состояния узла Tailscale теперь ездит вместе с узлом: переименование, перенос в папку и из папки, смена тег-политики папки переносят каталог, а не бросают его — идентичность устройства в tailnet (и сгоревший одноразовый ключ) сохраняется; удаление узла или его папки уносит каталог, а каждая сборка подчищает каталоги узлов, которых больше нет. Каталог, заданный вами вручную в теле узла (`state_directory`), не трогается.
