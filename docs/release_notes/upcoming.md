...
- The shared LX contract is drafted at **1.0** — the data agreement with the phone app (LxBox) now describes the 1.0 backup: `contract/schema/backup.schema.json` (marker `lx_backup: 2`, one `sources[]` section with a `kind` discriminator), with the previous schema kept as `backup-0.12.schema.json` so legacy files stay checkable; `docs/BACKUP.md` §11 lists every key that moved, `docs/NODE_SECTIONS.md` and `docs/ONE_NAMESPACE.md` are no longer drafts, and the corpus gained four 1.0 backup cases plus two whole-config body cases. `contract/VERSION` stays at 0.12.11 on purpose: it goes to 1.0.0 only once LxBox both reads and writes the new format, which is why the launcher still writes 0.12 by default.
...
- Общий контракт LX доведён до черновика **1.0** — соглашение о данных с мобильным приложением (LxBox) описывает формат бэкапа 1.0: `contract/schema/backup.schema.json` (маркер `lx_backup: 2`, одна секция `sources[]` с видом записи), прежняя схема сохранена как `backup-0.12.schema.json`, чтобы старые файлы по-прежнему проверялись; `docs/BACKUP.md` §11 перечисляет каждый переехавший ключ, `docs/NODE_SECTIONS.md` и `docs/ONE_NAMESPACE.md` вышли из черновика, в корпус добавлены четыре кейса бэкапа 1.0 и два кейса тела с целым конфигом. `contract/VERSION` намеренно остаётся 0.12.11: до 1.0.0 он поднимется, только когда LxBox научится и читать, и писать новый формат, — поэтому лаунчер по умолчанию пока пишет 0.12.
