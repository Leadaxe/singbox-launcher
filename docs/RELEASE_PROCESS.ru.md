...
**🌐 Язык**: [English](RELEASE_PROCESS.md) | Русский
...
4. **Pinned dependencies** (см. §5):
   - **`internal/constants.RequiredCoreVersion`** — соответствует версии форка `sing-box-lx`, на которой проводилось финальное QA релиза (fork-тег вида `X.Y.Z-lx.N`). Если вышел новый форк-релиз (или в upstream SagerNet появился критичный CVE/breaking-fix, который форк ребейзит) и решили бампнуть — отдельный коммит `chore(core): pin sing-box-lx X.Y.Z-lx.N` **до** тега, плюс пометка в release-notes. Win7 (`windows/386`) теперь тоже на форке (ассет `windows-386-legacy-windows-7`, с lx-сборки) — отдельной константы нет. Подробности — §5.1.
...
- **Источник ядра (SPEC 072):** теперь форк **`Leadaxe/sing-box-lx`** (`constants.SingboxCoreRepo`) — он собирает XHTTP (`with_xhttp`) и AmneziaWG (`with_awg`). Версия — fork-тег вида `X.Y.Z-lx.N` (форк-бинарь печатает полный тег в `sing-box version`, поэтому строгое сравнение в Core Dashboard работает). **Windows 7 (`windows/386`)** форк теперь собирает (ассет `windows-386-legacy-windows-7`, с `v1.14.0-lx.1-rc.17`) → тоже на форке, без отдельного upstream-пути (SourceForge/legacy-машинерия удалена). Ядро качается напрямую из GitHub-релиза по HTTPS — отдельной SHA256-сверки нет (источник доверенный, целостность канала обеспечивает TLS).
...
- **Когда менять:** при выходе нового форк-релиза `vX.Y.Z-lx.N` (новые фичи/фиксы XHTTP/AWG, ребейз на upstream-тег) — авто-дискавери нет (by design SPEC 046), поднимать константу вручную. Win7 (`windows/386`) — на той же `RequiredCoreVersion` (отдельной Win7-константы больше нет).
- **Как менять:**
  ```bash
...
### 5.3 Обновление шаблона на стороне пользователя

При апгрейде лаунчера `core.RefreshTemplateIfStale` (запускается в фоне из `main.go` через `StartTemplateRefresh`) сравнивает `Settings.LastTemplateLauncherVersion` (записан после последнего успешного скачивания шаблона) с `constants.AppVersion`. Если меньше — скачивается шаблон, закреплённый за этим релизом, и атомарно встаёт на место; старый `bin/wizard_template.json` лежит до этого момента, а при неудачном скачивании остаётся (следующий запуск пробует снова). Сборка конфига ждёт обновление, поэтому первый старт ядра после апгрейда пересобирается из нового шаблона; если пересборка падает, старт отменяется с диалогом ошибки, а не запускает прежний `config.json`.

Dev-сборки (AppVersion = `v-local-test`, `unnamed-dev`, `*-dirty`) проверку пропускают — иначе локальная разработка ломается на каждом запуске.
