...
| `chain`       | object   | Нет          | **SPEC 110.** Делает этот источник цепочкой хопов вместо подписки или сервера: `hops` (позиции в порядке движения пакета), `idle_timeout`, `interrupt_exist_connections` (рвать живые соединения пользователя при переключении позиции тумблером в рантайме, SPEC 124), `strip_evasion`, `strip`, `rewrite`. У такого источника нет ни `source`, ни `connections` — он материализуется в один outbound типа `chain`, тег которого берётся из `tag_mask`. Требует ядро, собранное с `with_lx_chain`. См. [Цепочки хопов](#цепочки-хопов). |
