...
| `detour_tag` | string | Нет | **SPEC 077.** Тег другого outbound, через который dial'ятся **все** узлы этого источника (proxy-цепочка / hop): узлу проставляется `"detour":"<tag>"` (`A через B`). Пусто = прямое соединение. Хранится по тегу (как правила/селекторы). Не применяется к WireGuard-узлам и к узлам с собственной Xray-цепочкой (`dialerProxy` побеждает). **Fail-open** на этапе генерации: самоссылка и цикл среди узлов (`A.detour=B`, `B.detour=A`) детектируются и разрываются с предупреждением (узел работает напрямую) — иначе ядро отвергло бы конфиг с ошибкой. Висячий detour на тег **шаблонной/preset-группы** не отбрасывается (теги групп известны только при финальной сборке). **UI-выбор намеренно сужен** (`business.DetourOptions`): предлагаются только **ручные глобальные selector-группы** (с вкладки Outbounds) и **активные preset-группы**; исключены встроенные/служебные (`direct-out`/`reject`/`drop`), auto-группа (`auto-proxy-out`), локальные группы самой подписки и отдельные узлы. Одиночные серверы как цель пока не добавлены. |
| `disabled_nodes` | object | Нет | **SPEC 094 D4.** Отметки о выключенных пользователем узлах: `{"<хеш идентичности>": <unix-время>}`. Ключ — `config.NodeIdentityHash`: sha256 от эмитированного outbound-JSON узла без полей `tag` и `detour`, с рекурсивно отсортированными ключами. Ключом взят хеш, а не тег и не позиция: провайдеры переименовывают узлы между обновлениями и свободно меняют их порядок, поэтому отметка по тегу молча переехала бы на другой сервер. Хеш покрывает всё, что описывает подключение (включая SNI и транспорт), поэтому отметка следует за конкретным узлом и переживает обновление подписки, перезапуск и переименование. Значение — время последнего подтверждения: отметка узла, отсутствующего в подписке дольше TTL `clamp(3 × интервал обновления, 24h, 30d)`, удаляется, иначе карта росла бы неограниченно. GC выполняется **только после успешного сетевого обновления** — на прогоне из кэша тело может быть неполным. |
