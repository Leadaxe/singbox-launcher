...
| **ADR-070-6** | Двунаправленная логика протоколов единственна в своём роде — через spec-билдеры. | **Снято SPEC 133** (запись выше — решение 2025 года, оставлена историей). Единственность достигнута, но не билдерами в Go: оба направления и оба входа читают ОДНУ таблицу реестра (`contract/registry/**/mappers.*`) одним движком `core/config/linkmap`. Файлы по протоколам (`node_parser_*`, `shareuri_*`) ушли вместе с рукописной логикой; объединять `TransportSpec`/`TLSSpec` больше не нужно — двух билдеров нет. |
...
> **Две позиции стадии D — история, а не текущая раскладка.** Разбиение
> `node_parser.go` и `share_uri_encode.go` по протоколам сделало копии правил
> видимыми, но оставило их: одно и то же правило жило в парсере ссылки, в
> конвертере Xray и в эмиттере, и они разъезжались. SPEC 133 удалил оба семейства
> файлов — источник узла разбирает один движок `core/config/linkmap` по таблицам
> реестра, в обе стороны.
...
| ~~**Объединение транспорта и TLS** — слить `uriTransportFromQuery` и `xrayTransportFromStreamSettings` в один билдер `TransportSpec`, а три TLS-билдера — в один `TLSSpec`.~~ | ADR-070-6 | **Больше не отложено — снято SPEC 133.** Оба рукописных пути удалены: вход ссылки и вход Xray ведёт одна таблица реестра через `core/config/linkmap`, сливать нечего. Риск round-trip, о котором говорит строка, теперь закрыт двумя раннерами — снимком ссылки байт в байт и кругом `parse(emit(body)) == body` по всему корпусу. |
