...
**🌐 Язык**: [English](PRODUCT_ANALYSIS.md) | Русский
...
**Десктоп-платформа сетевой маршрутизации и анализа трафика. 13 VPN-протоколов, глубина настроек и API уровня Enterprise. Поверх sing-box как execution-engine.**
...
2. **Headless mode из коробки.** Bearer-auth HTTP API из 28 путей (27 protected + unauthenticated `/ping`), документированный MCP integration path (SPEC 038 §6.5). Пригоден для DevOps-скриптов и AI-агентов: Claude через MCP читает `/state`, переключает прокси, рестартит engine — без открытия UI.
...
### Поверхность API — 28 путей (27 protected + unauthenticated `/ping`) в 6 группах
