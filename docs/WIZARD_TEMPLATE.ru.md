...
**🌐 Язык**: [English](WIZARD_TEMPLATE.md) | Русский
...
- **Элемент списка:** `["@dns_list", ...]` где `dns_list` — `text_list` → его строки
  вливаются в родительский массив. Одноэлементный массив `["@name"]` **не** сворачивается в
  скаляр (SPEC 143): скобки автора сохраняются, что встанет на место ссылки, решает только
  объявленный `type` — `["@tun_address"]` при `text`-переменной даёт `["172.16.0.1/30"]`.
- **Тип результата** определяется только объявленным `type`: `bool` → `true/false`,
  `text_list` → массив, `int` → число (clamp в [0, 65535]; не-число остаётся строкой, в отчёте
  сборки — предупреждение), остальные → строка. Зашитого списка числовых имён нет: порт или MTU,
  объявленные как `text`, подставляются строкой.
- `"@runtime.platform"` / `"@runtime.arch"` / `"@runtime.target"` разрешены в значениях
  `config` (SPEC 143); неизвестное поле `@runtime.*` — ошибка загрузки.
- Имя после `@` не содержит пробелов; неизвестный `@var` в `config` → ошибка загрузки.

`@var` как **ключ** объекта используется только в predicate'ах `#if` (§4), в обычном
...
| `options` | array | Допустимые значения, `["a","b"]` или `[{"title","value"}]`; при любом типе, кроме `bool` (ошибка загрузки). Объектная форма тип не меняет. |
| `options_open` | bool | `true` — можно ввести значение вне `options` (комбобокс). Без `options` игнорируется. |
...
|---|---|---|
| `text` | поле ввода | строка |
| `int` | поле ввода с проверкой числа | число, clamp в [0, 65535]; не-число остаётся строкой |
| `bool` | чекбокс | `true` / `false` |
| `enum` | = `text` с закрытыми `options` (читается бессрочно; в новых шаблонах писать `text` + `options`) | `value` выбранного варианта |
...
`options` ортогональны `type` (SPEC 143): `type` говорит, что уедет в JSON, `options` — какие
значения допустимы. С `options` строка — выпадающий список (объектная форма показывает `title`,
хранит `value`); при `options_open: true` — комбобокс со свободным вводом. `int` проверяется как
число и в обычном поле, и в комбобоксе.
...
**`#if`** (§4) вычисляется в runtime; внутри разрешены **только** `@runtime.*` globals,
...
> Подробнее: парсер подписок — [`ParserConfig.md`](ParserConfig.md); хранение в state
> (`vars`, `dns_options`, правила) — [`WIZARD_STATE.md`](WIZARD_STATE.md); движок и формат
> stock-JSON — [`TEMPLATE_REFERENCE.md`](TEMPLATE_REFERENCE.md).
