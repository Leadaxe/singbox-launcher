...
| `chain_cycle.go` / `detour_group_cycle.go` | Кольца «Направление ↔ цепочка» и «узел ↔ группа, через которую он ходит»: fail-open, элемент выпадает из состава, а не роняет конфиг. |
