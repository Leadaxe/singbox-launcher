...
| `outbound_graph_sanitize.go` | **Финальный проход по графу зависимостей.** Один обход всех видов рёбер (участник группы / detour / позиция цепочки) до фикспойнта: висячие ссылки, кольца через рёбра разных видов, опустевшие группы, `default` вне состава и инварианты цепочек, которые ядро проверяет только на `run`. Деградирует один элемент с warning, вместо того чтобы отдать ядру конфиг, который оно отвергнет целиком. **Новый вид ребра добавляется сюда, а не четвёртой частной проверкой.** |
| `tls_transforms.go` | **SPEC 092.** Опциональные анти-DPI TLS-преобразования (fragment / record fragment / смешанный регистр SNI) для outbound'ов первого хопа перед эмиссией. |
| `sync_dns.go` | Синхронизация списка DNS между UI-моделью и каноническим состоянием. |
...
| `chain_nodes.go` | **SPEC 110.** Источники-цепочки превращаются в узлы здесь, когда весь пул уже загружен: позиции ссылаются на теги, окончательные только после разбора всех источников. Цепочка ссылается лишь на цепочку, объявленную ВЫШЕ, — так циклы невозможны по построению. Не прошедшая проверку цепочка не становится узлом вовсе. |
| `chain_generator.go` | Эмиссия объекта `type: chain` (позиции в порядке пакета, `strip`, `rewrite`, `idle_timeout`). |
| `chain_validate.go` | Инварианты, которые `sing-box check` не ловит (падает только `run`): ключ `strip`, который тело звена требует (`ChainUnstripRequired` — `on_hop_required` реестра, судит `nodeflow.StripBlocked`; `tls.utls` у REALITY-звена не снимается, цепочка собирается), вложенная цепочка не на позиции 0. Плюс `ChainLayerTag` — схема служебного тега `<chain>#<i>` для послойной пробы. |
| `chain_cycle.go` / `detour_group_cycle.go` | Кольца «Направление ↔ цепочка» и «узел ↔ группа, через которую он ходит»: элемент выпадает из СОСТАВА группы, а не роняет конфиг. Detour при этом **не снимается** — узел остаётся в конфиге и продолжает ходить своим переходом (SPEC 113-B: снятие ключа = тихий прямой дозвон, запрещено). Это не fail-open для самого перехода: недоступная цель detour разбирается отдельно и выбрасывает носителя. |
| `folder_replaces.go` | **SPEC 118.** Разворачивание свёртки папки/подписки (`replace {mode: manual\|auto\|both, tag, auto?}`) в локальные группы на сборке — селектор, автогруппа или селектор с двойником `<tag>-auto` (тот же суффикс, что у твинов Направлений); в состоянии они не хранятся. Тег свёртки, уже объявленный Направлением, свёрткой выше или тегом шаблона, не собирается (`replace_tag_conflict`); теги построенных свёрток занимаются в счётчике финальных тегов раньше узлов, и узел-тёзка получает `-2`. |
...
| `xray_json_array.go` / `xray_element_engine.go` / `xray_outbound_convert.go` / `xray_protocols.go` / `xray_balancer.go` | JSON-массив Xray на уровне ДОКУМЕНТА: какой элемент становится узлом, какой звеном цепочки (`xrayChainHopFromOutbound`; звено переводит та же секция движка, что и узел), какой группой-балансером; `remarks`→Label, slug-теги, владение сервером. Сам элемент переводит движок (`parseXrayElementByEngine`). |
...
| Файл | Назначение |
|------|------------|
| `types.go` | Форма файла: подписки, серверы, цепочки, Направления, правила, DNS, переносимые переменные. Механизма `extensions` нет (контракт 0.11.0): файл — сериализация состояния и ничего кроме. |
| `export.go` / `export10.go` | `State` → бэкап формата 1.0 (единственный писатель с v1.6.0, D-110), **чистая функция состояния**: поля источника (skip-фильтры, detour, id, тег узла, свёртка `replace {mode, tag, auto?}`) — обычные необязательные ключи записи. Два неотличимых состояния обязаны давать байт-идентичные файлы, поэтому следов «откуда взялось» в файле не бывает. |
| `import.go` | Бэкап → `State`. Правило, чья цель здесь не существует, импортируется **выключенным**, а не теряется и не включается: включённое правило с мёртвой целью заставляет ядро отвергнуть весь конфиг. Всё, чего импортёр не понимает, отбрасывается с warning'ом и никуда не провозится — состояние после импорта неотличимо от настроенного руками. |
...
| `backend_daemon.go` (+ `_dns`, `_traffic`, `_tailscale`) | `DaemonBackend` — движок lxd: управляющая плоскость gRPC, структурные стримы DNS, соединений и tailnet для профайлера. Daemon-платформы: `darwin \|\| (windows && !386)` (SPEC 141). |
...
| `backend_daemon_stub.go` (+ `debugapi_wiring_daemon_stub.go`, `purge_other.go`) | Заглушки для Linux и Win7 (`!darwin && (!windows \|\| 386)`), чтобы остальной код компилировался без движка (именно они держат сборку под Win7 чистой). |
| `daemon_manager.go` (+ `_darwin.go`, `_windows.go`) | Жизненный цикл демона со стороны лаунчера: строки команд, которые он отдаёт пользователю (`--service=install` / `=uninstall [--purge]` / `lxd client add`), сопряжение, паспорт демона (`GET /admin/info`), подготовка конфига, диалог после обновления ядра. `_darwin.go`: метка и plist launchd, bootstrap / kickstart, рендер sudo, Terminal; `_windows.go`: заглушки точек расширения, движок закрыт (SPEC 141). Сам ничего привилегированного не запускает. |
| `daemon_service_state.go` (+ `_darwin.go`, `_windows.go`) | **SPEC 136.** Классификатор службы: читает plist launchd, проверяет root-owned копию ядра и цепочку владения (`/Library` → `PrivilegedHelperTools` → плоский файл копии), сверяет sha256 с ядром лаунчера (кэш по dev/inode/size/mtime) и с паспортом работающего демона. Вердикты: не установлена / небезопасна / устарела / процесс устарел / в порядке и «ядро лаунчера ниже lx.12» — команды install и copy гейтятся версией ядра лаунчера (≥ 1.14.1-lx.12, включая его rc). Только чтение, без sudo. Общее: состояния, гейт версий, вердикты sha и процесса, кэши хэшей и версий; `_darwin.go`: раскладка `/Library`, plist, звено цепочки по uid (`Stat_t`), ключ кэша, launchd; `_windows.go`: заглушки (служба «не установлена»). |
| `classic_privileged.go` (+ `_darwin.go`, `_windows.go`, `_other.go`) | **SPEC 137.** Гейт привилегированного старта classic (TUN): root-owned копия ядра есть, проходит цепочку владения и совпадает с ядром лаунчера по sha256 (функции и кэш хэшей `daemon_service_state.go`); тексты диалога — в `_darwin.go`; иначе — диалог с одной sudo-командой (`lxd --service=copy`, при установленной службе — `--service=install`) и Retry; WARN после скачивания ядра, если копия отстала. Только чтение, без sudo. |
...
**Ответственность:** представления вкладок визарда (Sources / Directions / Rules / DNS / Settings / Generate / Files, плюс Target для удалённой машины) и окно правки источника.
...
| `source_meta_format.go` / `source_support_link.go` | Форматирование метаданных источника, ссылка на поддержку и веб-страницу. |
| `source_replace_tab.go` | **SPEC 118.** Вкладка «Группа» окна источника: свёртка (`replace`) — селектор / автогруппа / селектор с автогруппой по умолчанию, плюс явный тег замены (позиционного тега больше нет). |
| `source_chain_tab.go` / `source_chain_hops.go` / `source_chain_rewrite.go` | **SPEC 110.** Форма цепочки хопов: позиции в порядке пакета (перетаскивание), пикер кандидатов (узел / группа / Направление / служебный тег), таблица `rewrite` (протокол · ключ · JSON-значение, `null` удаляет). Форма отвергает комбинации, которые ядро отвергает только на `run`, — здесь она единственный рубеж. |
| `target_tab.go` | Вкладка Target удалённой машины (SPEC 097): режим gateway и LAN-интерфейсы. |
| `files_tab.go` | Вкладка Files: сборка `config.json` в файл, просмотр в отдельном окне только для чтения. |
| `settings_reactive.go` | **SPEC 107.** Точечный пересчёт гейтов на вкладке Settings: статический индекс зависимостей обновляет только те строки, чьё условие реально изменилось. |
| `settings_backup.go` | UI экспорта и импорта LX Backup (SPEC 103, фаза 4). |
...
| `row_scaffold.go` | Общий каркас строки для переупорядочиваемых списков Rules/DNS/Sources: `buildRowLeftLead` (↑/↓ и чекбокс), `buildRowEditDelCluster` (иконки правки/удаления), `newRowLabelToggleTap` (переключение по тапу на подпись), `finalizeRow` (сборка Border + HoverRow + hover подсказки). Сводит отступы иконок в одно место, чтобы они не разъезжались от билдера к билдеру. |
