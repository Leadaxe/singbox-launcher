...
| `chain_validate.go` | Инварианты, которые `sing-box check` не ловит (падает только `run`): reality + снятый `tls.utls`, вложенная цепочка не на позиции 0. Плюс `ChainLayerTag` — схема служебного тега `<chain>#<i>` для послойной пробы. |
...
| `source_folds.go` | **SPEC 108.** Разворачивание свёртки подписки в группы на сборке (в состоянии они не хранятся). |
...
| `node_parser_transport.go` | Дописывание uTLS под REALITY на сборке (`EnforceRealityFingerprint`). Значения отпечатка uTLS и WS early data — правила реестра, их исполняет движок. |
...
| `export.go` | `State` → бэкап, **чистая функция состояния**: поля источника (skip-фильтры, локальные outbound'ы, detour, id, тег узла, свёртка) — обычные необязательные ключи записи. Два неотличимых состояния обязаны давать байт-идентичные файлы, поэтому следов «откуда взялось» в файле не бывает. |
...
| `source_fold_tab.go` | **SPEC 108.** Вкладка «Группа» окна источника: одна галка свёртки вместо прежних четырёх флагов; выбор селектора / автовыбора / селектора с автогруппой по умолчанию. |
