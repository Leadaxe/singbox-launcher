...
- **⚠️ REALITY: явный отпечаток сохраняется, пустой становится chrome (D-119).** Везде, где реально эмитится блок `tls.reality`, uTLS включается всегда (без него ядро REALITY не поднимает). Отпечаток, который узел задал явно, уходит в `config.json` как есть; пустой `fp` и наш собственный неявный дефолт `random` пишутся как `chrome`. REALITY-сервер на Xray ≥ v26.9.8 требует в ClientHello `key_share X25519MLKEM768`, который в uTLS ядра несут chrome-спеки, а с ядра `1.14.1-lx.3` ещё `firefox` и `safari`; поэтому узел с явным отпечатком вне этого набора (`ios`, `android`, `edge`, `360`, `qq`) получает код `reality_fp_not_chrome`: если соединение не устанавливается — попробуйте `chrome`. Сам отпечаток за вас не подменяется: это выбор подписки, и лаунчер делает так, как она велит (D-104 с принудительным chrome заменён).
- **`key_share` — обмен ключами REALITY (ядро ≥ `1.14.1-lx.4`, D-121).** Параметр ссылки `key_share=hybrid|classical` у `vless://` и `anytls://` рядом с `pbk`/`sid` — имя ключа sing-box как есть. `hybrid` требует `X25519MLKEM768`, `classical` его срезает (это нужно REALITY-серверам Xray **старше** v26.9.8, которые на гибридном ClientHello рвут соединение); пусто = как несёт uTLS-отпечаток. Читается **только при валидном `pbk`** — на узле без REALITY поле смысла не имеет. Значение вне этих двух **отбрасывается** с кодом `reality_key_share_invalid` (узел живёт): ядро отвергает такое значение отказом ВСЕГО конфига, а не одного узла. На ядре старше `1.14.1-lx.4` поле не эмитится вовсе — ключ ему неизвестен, а неизвестный ключ роняет конфиг целиком; узел при этом остаётся REALITY.
