...
В Windows-архивах релиза рядом с `singbox-launcher.exe` лежат два батника
(исходники — `build/windows/`):

- `autostart_add.bat` — создаёт задание Планировщика с именем `singbox-launcher`, которое
  запускает лаунчер при входе в систему с наивысшими правами:
  `schtasks /Create /TN "singbox-launcher" /SC ONLOGON /RL HIGHEST /TR "<папка>\singbox-launcher.exe" /F`
- `autostart_remove.bat` — удаляет это задание (`schtasks /Delete /TN "singbox-launcher" /F`).

Оба нужно запускать **от имени администратора** и держать рядом с `.exe` — путь они берут из
своего расположения (`%~dp0`). Ярлык в папке «Автозагрузка» не равноценен: манифест лаунчера
требует прав администратора, поэтому ярлык даёт запрос UAC при каждом входе, а задание
Планировщика с `/RL HIGHEST` стартует молча и раньше обычных автозапускаемых приложений.

Чтобы лаунчер стартовал свёрнутым и с поднятым VPN, откройте действие задания и допишите к
команде `-start -tray`. Если лаунчер зависает при входе в систему, добавьте триггеру «При
входе в систему» задержку 30 секунд — 1 минуту: сессии и видеодрайверу нужно время на
инициализацию (issue #79).
