...
const (
	...
	// heartbeatTimeout — сколько ждём ответа машины на один тик.
	//
	// Заведомо короче периода: опрос не имеет права переживать собственный
	// тик. С общим restTimeout (30 с) на КАЖДЫЙ из двух запросов Health
	// (/admin/status + /admin/info) один тик на зависшей машине держался до
	// минуты, и до красного маркера проходило под две минуты вместо
	// задуманных десяти секунд — то есть порог промахов не работал вовсе.
	//
	// 4 секунды: /admin/status — один дешёвый GET, и машина, которой не
	// хватило четырёх секунд, для строки списка и есть «не отвечает».
	// Ручные Reload/Connect ходят прежним путём, с полным сроком.
...
func (p *machineListPanel) pollActive() {
	...
	h := p.registry.HealthWithin(id, heartbeatTimeout)
	fyne.Do(func() {
		...
		if h.Err != "" {
			...
			//
			// Перерисовка — только на смене маркера (зелёный→жёлтый на первом
			// промахе, жёлтый→красный на пороге): redrawRows строит все строки
			// заново, и звать её на каждом тике значит дёргать панель без
			// видимой причины. Промахи дальше порога маркер не меняют.
			if live.FailStreak == 1 || live.FailStreak == heartbeatFailThreshold {
				p.redrawRows()
			}
			return
		}
		...
		p.health[id] = h
		// То же правило на успехе: тик, не изменивший ни health, ни маркер, —
		// не повод пересобирать строки. Иначе панель вздрагивает каждые 5 с.
		if !had || prev != h || recovered {
			p.redrawRows()
		}
