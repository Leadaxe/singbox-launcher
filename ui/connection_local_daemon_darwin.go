...
// buildRemoteDaemonPanel — REMOTE-вид окна подключения: демон на другой
// машине. Только то, что реально нужно клиенту: сопряжение по приглашению,
// адрес, секрет (для plain-демона) и Unpair. Установка/обслуживание/удаление
// службы — дело оператора того хоста, этих кнопок здесь нет.
func buildRemoteDaemonPanel(ac *core.AppController, win fyne.Window, onChanged func()) fyne.CanvasObject {
	hint := widget.NewLabel(locale.T("conn.remote_hint"))
	hint.Wrapping = fyne.TextWrapWord
	...
	refreshStatus := func() {
		go func() {
			snap := ac.DaemonStatusSnapshot()
			fyne.Do(func() {
				status.SetText(renderDaemonStatusText(ac, snap, false))
			})
		}()
	}
	refreshBtn := ttwidget.NewButtonWithIcon("", theme.ViewRefreshIcon(), refreshStatus)
	...
	pairBtn := widget.NewButton(locale.T("settings.daemon_pair_btn"), func() {
		...
		go func() {
			...
			fyne.Do(func() {
				...
				dialogs.ShowAutoHideInfo(ac.UIService.Application, win,
					locale.T("conn.window_title"), locale.T("settings.daemon_pair_done"))
				activateDaemonEngineIfPossible(ac)
				if onChanged != nil {
					onChanged()
				}
				refreshStatus()
	...
	addressEntry.SetPlaceHolder("10.0.0.2:9091")
	addressEntry.OnSubmitted = func(text string) {
		if err := ac.SetDaemonAddress(text); err != nil {
			ShowError(win, err)
			return
	...
	return container.NewVBox(
		...
		container.NewBorder(nil, nil, nil, refreshBtn, status),
		widget.NewSeparator(),
		wrappedLabel("conn.remote_pair_step"),
		container.NewBorder(nil, nil, nil, pairBtn, inviteEntry),
		secretEntry,
		container.NewBorder(nil, nil, widget.NewLabel(locale.T("settings.daemon_address_label")), unpairBtn, addressEntry),
	)
}
