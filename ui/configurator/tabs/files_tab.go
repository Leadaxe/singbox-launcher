...
const (
	filesHintText = "Save the settings: state is written and the working config.json is rebuilt — the same as the Save button below. \"Show config\" opens the built config.json in its own window without saving anything."
)
...
func CreateFilesTab(presenter *wizardpresentation.WizardPresenter, guiState *wizardpresentation.GUIState) fyne.CanvasObject {
	...
	hint := widget.NewLabel(locale.T(filesHintText))
