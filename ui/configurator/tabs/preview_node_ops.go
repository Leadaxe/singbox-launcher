...
// С W11 индексы, вообще говоря, совпадают — buildPreviewRows (preview_rows.go)
// строит строку на КАЖДЫЙ элемент `src.Nodes` в его порядке, включая
// выключенные и неразобранные. Но резолв по тегу оставлен намеренно: он не
// зависит от этого свойства списка, и день, когда превью начнёт фильтровать
// строки, не превратит перестановку в тихую порчу состава.
//
