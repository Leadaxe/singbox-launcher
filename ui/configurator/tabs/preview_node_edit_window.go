...
// preview_node_ops.go).
//
// # Почему переименование — это Save окна источника, а не своя команда
//
// Имя узла контейнера — его идентичность (SPEC 112), и смена тега тянет за
// собой перепись ссылок (`RepointContainerNodeLinks`), переезд каталога
// состояния tailnet (SPEC 122 норма 2), разыменование от подписки (Д5) и
// предупреждение о протухшем выборе. Своей команды Rename у списка узлов
// нет (обкатка заход 3): она вела к тому же действию, что поле формы, и два
// пути к одному только путали. Весь набор живёт в обработчике Save окна
// источника — ветка `editingNode` в `showSourceEditWindowAt`
// (source_edit_window.go): проверка занятости тега в контейнере, запись узла
// по его адресу (`applyNodeEditToModel`), затем перепись ссылок реестром W2.
// Это окно и открывается на узел (`showSourceEditWindowForNode`), а здесь
// строка списка лишь переводится в адрес `NodeLink`.
package tabs
