...
func InvalidateNodePool(model *wizardmodels.WizardModel) {
	...
	// SPEC 121: строки правил, которые узлы носят с собой, — такая же
	// производная от состава, и снимает их то же событие. Своей точки пересева
	// у них нет намеренно: «правка узла» случается в дюжине мест (окно
	// источника, папка, удаление, импорт бэкапа), и каждое из них уже зовёт
	// инвалидацию — а вот помнить про второй вызов пришлось бы каждому
	// (ловушка «кэши инвалидируются парой»).
