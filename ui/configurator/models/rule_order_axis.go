...
func (m *WizardModel) applyAxisAfterMove(movedPos int) {
	...
	if movedPos == 0 {
		// Самый верх списка: соседа сверху нет. Начало пользовательской зоны
		// (1000) тут не годится — над правилом могут стоять сортируемые якоря
		// 950..990, и оно уехало бы визуально вверх, а по маршрутизации вниз.
		// Встаём ПЕРЕД соседом снизу, вытесняя его ленивым сдвигом: равных
		// номеров не остаётся, тай-брейк снова только тай-брейк.
	...
	// Закон оси: массив всегда отсортирован по номерам. Клэмп M7 и ленивый
	// сдвиг могут увести правило не туда, куда его бросили (дроп под якорь 950
	// не даёт номер 951 — зоны 1..949 нет), и без этой пересортировки список
	// показывал бы одно, а маршрутизация делала другое до следующей загрузки.
