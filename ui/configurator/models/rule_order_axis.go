...
// «встал самым первым» разбирается отдельно — см. ниже.
//
// Ленивый сдвиг D-053а живёт внутри PlaceRuleAfter и здесь не дублируется:
...
func (m *WizardModel) applyAxisAfterMove(movedPos int) {
	...
	if movedPos == 0 {
		// Самый верх списка: соседа сверху нет, и PlaceRuleAfter отправил бы
		// правило в начало ПОЛЬЗОВАТЕЛЬСКОЙ зоны (1000) — ниже шаблонных
		// якорей 950..990, то есть визуально вверх, а по маршрутизации вниз.
		// Отдаём номер соседа снизу: равные номера разрешены, а стабильная
		// сортировка оставляет перетащенное правило первым.
		if len(proxy) > 1 && proxy[1].OrderNum != nil {
			n := *proxy[1].OrderNum
			proxy[0].OrderNum = &n
		}
	} else {
