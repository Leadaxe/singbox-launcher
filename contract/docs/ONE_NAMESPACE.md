...
чтение и запись бэкапа 1.0 — релиз 2.23.3, синхронный с лаунчером 1.6.0. С
...
5. ✅ LxBox — чтение и запись бэкапа 1.0 + legacy-чтение файлов 0.x, релиз
   2.23.3 (задача — `TASKS_LXBOX.md` `## 16`).
6. ✅ `contract/VERSION` → 1.0.0 (D-110): обе стороны читают и пишут новую
   ...
   0.12»: релизы лаунчера 1.6.0 и LxBox 2.23.3 синхронны, с них обе стороны
