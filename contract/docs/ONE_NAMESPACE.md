...
> **О чём этот документ.** Правило одной формы записи: узел, правило
> маршрута, DNS-сервер и DNS-правило записываются одинаково в состоянии
> лаунчера, в состоянии LxBox и в файле бэкапа — как метаданные приложения
> плюс `body`, объект sing-box как есть. Благодаря этому бэкап является
> состоянием без переводчика между формами, а две реализации читаются
> одними глазами. Здесь сверка трёх сторон по каждой сущности, целевые
> формы и порядок, в котором к ним пришли.
>
> **Кому читать.** Тому, кто добавляет новую сущность или поле в состояние
> любой стороны: форма обязана быть одной для всех трёх мест.

Статус: **норма контракта 1.0.0**. Целевые формы §1–§2 и порядок §4 утверждены
владельцем 14.09.2026 (D-106, D-107); контракт поднят до 1.0.0 решением D-110. Цель: одна форма записи для каждой сущности во всех
...
| 2 | бэкап 1.0 (`lx_backup: 2`) = сериализация состояния; два писателя (второй, 0.12, снят D-110), единое слияние, legacy-вход 0.x | `768ef591`, влито `9fc880e1` |
| 3 | контракт: схема `backup.schema.json`, `backup-0.12.schema.json`, документы, корпус | `05d4f35f`, влито `2c3a16c8` |
| — | лаунчер 1.6.0 пишет бэкап только в 1.0 — писатель 0.12 и выбор формата сняты (D-110) | ветка `spec127/backup-10-only` |
...
чтение и запись бэкапа 1.0 — релиз 2.24.0, синхронный с лаунчером 1.6.0. С
этих релизов обе стороны пишут только 1.0 и читают 0.x как legacy (D-110);
`contract/VERSION` = 1.0.0 (§4 п. 6).
...
> Ссылка на узел (`detour`, `hops[]`, `group.members[]`) во всех трёх местах — NodeLink `{folder_id, tag}`: норма `NODE_LINK.md` (D-112).
...
5. ✅ LxBox — чтение и запись бэкапа 1.0 + legacy-чтение файлов 0.x, релиз
   2.24.0 (задача — `TASKS_LXBOX.md` `## 16`).
6. ✅ `contract/VERSION` → 1.0.0 (D-110): обе стороны читают и пишут новую
   форму. Решением владельца (D-110) отменено окно «два писателя, дефолт
   0.12»: релизы лаунчера 1.6.0 и LxBox 2.24.0 синхронны, с них обе стороны
   пишут только 1.0, а 0.x читают всегда. Приёмка SPEC 126 §4 (бэкап с узлом
   Tailscale десктоп ↔ телефон) — на 1.0.
