...
Статус: **целевые формы §1–§2 и порядок §4 утверждены владельцем 14.09.2026 (передано сессией LxBox) с тремя уточнениями, внесены; ждёт подтверждения владельца в сессии лаунчера, после чего документ становится нормой контракта 1.0** (D-106, D-107).
...
3. Контракт 1.0: схема + документы + корпус (лаунчер), решение D-109 после
   реализации; `## 12` (`refs[]`) закрыт D-107 по варианту А — слово владельца.
