...
- **`userinfo`** — User identifier.
  - Type: `uuid`
  - Maps to: `uuid`
- **`flow`** — XTLS flow control mode.
  - Type: enum: `""`, `xtls-rprx-vision`, `xtls-rprx-vision-udp443` · Default: `""`
  - Maps to: `flow`
  - If invalid: If invalid: removed → [`flow_deprecated`](../warnings.md#flow_deprecated)
- **`packetEncoding`** — How UDP packets are encapsulated.
  - Also spelled: `packetencoding`
  - Type: alias_enum: `""`, `xudp`, `packetaddr` (allowlist `packet_encoding`) · Default: `""`
  - Maps to: `packet_encoding`
  - If invalid: If invalid: removed → [`packet_encoding_unknown`](../warnings.md#packet_encoding_unknown)
- **`encryption`** (mobile) — Post-quantum encryption layer.
  - Type: string · Default: `""`
  - Maps to: `encryption`
- **`type`** — Transport selector.
  - Type: enum: `""`, `tcp`, `raw`, `ws`, `grpc`, `http`, `h2`, `httpupgrade`, `xhttp` · Default: `""`
  - Maps to: `transport.type`
- **`headerType`** — Xray TCP/RAW HTTP camouflage selector.
...
- **`encryption`** — Post-quantum encryption layer.
  - Type: string
  - Requires: core ≥ `1.14.0-lx.18`, lx fork only
...
- [`field_conflict`](../warnings.md#field_conflict)
  - `flow` — conflicts with `transport` → removed
...
- `encryption` — core ≥ `1.14.0-lx.18`, lx fork only
