...
- **Tailscale** — лаунчер сам умеет быть узлом tailnet: **Add server → Tailscale** собирает endpoint `tailscale` (auth key, control URL, hostname, ephemeral, accept routes, exit node) вместе с его MagicDNS-сервером (`*.ts.net`) и правилом маршрута на `100.64.0.0/10` — после Rebuild имена tailnet резолвятся, а хосты доступны. У каждого узла свой каталог состояния в `bin/tailscale/<тег>`. Нужно ядро с тегом `with_tailscale` — `1.14.0-lx.31` или новее; на старом ядре такой узел выбрасывается с предупреждением в отчёте сборки, а не валит весь конфиг. Ключ одноразовый: расходуется при первом входе, дальше идентичность машины держит каталог состояния. В Направление узел tailnet берите только с `exit_node` — без него он открывает саму tailnet, а не выход в интернет.
...
2. Задачи по желанию: ярлык на рабочем столе, «Запускать вместе с Windows», программный OpenGL (Mesa3D) для RDP / ВМ без видеокарты.
...
2. Распакуйте в папку, куда может писать ваша учётная запись (например, `D:\Tools\singbox-launcher` или папку в профиле): в архиве лежит `portable.txt`, и настройки остаются рядом с программой. `C:\Program Files\…` тоже подойдёт, но писать туда может только администратор, поэтому маркер там не учитывается, и данные живут в `%LOCALAPPDATA%\singbox-launcher` (см. [Где лежат данные](#где-лежат-данные)).
3. Запустите `singbox-launcher.exe`. Он стартует **без прав администратора**, без запроса UAC.
...
**TUN и права администратора.** Режиму прокси (`proxy-in` с системным прокси) права не нужны. TUN (по умолчанию) создаёт сетевой адаптер и меняет маршруты — Windows разрешает это только администраторам: **Start** без прав показывает диалог — **Перезапустить от имени администратора** (один запрос UAC; лаунчер перезапустится с правами, с теми же данными, и поднимет VPN) или **Перейти в режим прокси** (выключает TUN и включает системный прокси). У окна с правами в заголовке — `(Администратор)`. При включённом TUN лаунчер запрашивает эти права сразу при старте (один запрос UAC, `-tray`/`-start` сохраняются); выключается в Settings → Connection → **Запрашивать права администратора при старте, если включён TUN**.
...
Переход с portable-копии на установщик: в старой копии снимите **Settings → Storage → Portable mode** (данные переедут в `%LOCALAPPDATA%\singbox-launcher`), затем ставьте — или перенесите настройки через LX Backup. Если старая копия ещё на диске, **Открыть файл…** в диалоге **Загрузить** (Read) конфигуратора загрузит её `bin\wizard_states\state.json` (или любой снапшот оттуда), а **Импорт из файла…** в окне **+ Добавить** на вкладке Remote добавит её удалённые серверы из `bin\remote-daemons.json`.
...
**Удалить лаунчер и его данные**: саму папку программы (исполняемый файл, на macOS — `.app`) удаляйте как угодно — лаунчер её не трогает. Чтобы аккуратно удалить ещё и данные с логами, используйте **Settings → Storage → Remove all data…** либо `singbox-launcher -purge-data` для просмотра плана (добавьте `-yes`, чтобы выполнить). В обоих случаях VPN должен быть остановлен. На Windows заодно удаляется запись **автозапуска**, если она указывает на эту копию. Без прав администратора сетевая очистка (призрачные адаптеры, правила брандмауэра) и остатки в защищённой папке программы пропускаются с пометкой «Нужны права администратора». Оставшаяся в `Program Files` старая копия данных при следующем запуске снова скопируется в `%LOCALAPPDATA%`; `"<exe>" -purge-data -yes` из командной строки администратора доделает очистку.
...
| Windows: Start показывает «Для TUN нужны права администратора» | Лаунчер работает без прав администратора, а TUN они нужны. Выберите **Перезапустить от имени администратора** или **Перейти в режим прокси** — см. [Установка → Windows](#windows-1). |
