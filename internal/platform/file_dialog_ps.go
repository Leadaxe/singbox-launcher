...
//
// Лаунчер на Windows работает с правами администратора, а подпись диалога
// приходит из перевода (ru.json в каталоге данных — его может переписать
// процесс обычной целостности). Поэтому значения в код PowerShell строками
// не вставляются вовсе: скрипт целиком собирается здесь, а каждое значение —
