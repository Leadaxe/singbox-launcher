...
type WireEvent struct {
	...
	When time.Time
	// Kind — вид обмена. Сейчас в журнал попадает только «REST»: единственный
	// производитель событий — Client.record, а gRPC-плоскость (DialGRPC и
	// стримы поверх неё) своих отметок не пишет. Поле остаётся строкой, а не
	// флагом, чтобы её появление не потребовало переписывать читателей.
	Kind string
...
// RecordWire дописывает событие в журнал машины.
//
// Открыто наружу под будущие не-REST записи: стримы gRPC живут в services и
// смогут отмечаться сами. ВЫЗЫВАЮЩИХ ПОКА НЕТ — весь журнал наполняется
// только через Client.record, и всё, что в нём видно, это REST-обмен.
