...
//
// Список самого GLFW нужен потому, что в лаунчере он не обновляется. GLFW
// опрашивает мониторы при glfwInit (создание первого окна) и в
// applicationDidChangeScreenParameters своего делегата NSApplication, а
// platform.SetupDockReopenHandler ставит приложению свой делегат. Лаунчер,
// запущенный при спящем дисплее (например, с -tray), так и живёт с пустым
// списком: дисплей проснулся, CoreGraphics видит экран, а Show() всё равно
// упал бы.
