...
russian.TaskAutostart=Запускать вместе с Windows
...
english.StatusAutostart=Registering start with Windows...
...
english.RemoveDataQuestion=Also remove the launcher settings, subscriptions and logs?%n%nOnly the data of the account that runs the uninstaller is removed. Other users of this computer can remove theirs beforehand with Remove all data in the launcher settings.
russian.RemoveDataQuestion=Удалить также настройки, подписки и логи лаунчера?%n%nУдаляются данные учётной записи, от которой запущено удаление. Другие пользователи этого компьютера могут заранее удалить свои кнопкой Remove all data в настройках лаунчера.
