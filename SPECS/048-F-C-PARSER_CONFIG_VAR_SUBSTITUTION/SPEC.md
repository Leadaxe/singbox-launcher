...
**Тип:** F (Feature) · **Статус:** C (Complete) · **Релизы:**
- **v0.8.8.1** — initial hotfix, **устраняет критический баг** (sing-box не стартует на свежей установке из-за `@urltest_*` литералов в config.json).
- **v0.8.8.3** — follow-up, чинит **degraded feature** (Settings tab override молча теряется по пути в config) + path-инварианты + canary-тест.

Версия v0.8.8.2 была локально использована во время работы над follow-up'ом, но публикации не достигла — на ревью пользователь обнаружил третий баг (неверный JSON-ключ) ещё до создания тега, поэтому опубликован сразу v0.8.8.3.
...
## 3. Три бага в override-ветке v0.8.8.1

> **Важно:** v0.8.8.1 **устраняет критический баг** (литералы `@urltest_*` в config.json → sing-box check падает → лаунчер не стартует). После публикации жалующиеся юзеры были разблокированы.
>
> Ниже — баги в **degraded feature**: ветка резолюции «пользовательский override из Settings tab». Эта ветка отвечает за случай «юзер выставил `urltest_interval=1m` в UI, оно должно перебить template default `5m`». В v0.8.8.1 эта ветка молча игнорировалась — но template-default ветка работала корректно, поэтому видимая критическая поломка была покрыта.

### 3.1. Резолюция в три уровня

`BuildVarSubstituterFromDisk` спрашивает три источника по убыванию приоритета:

| Уровень | Что | v0.8.8.1 |
|---|---|---|
| 1 | `loadStateSettingsVars` — override юзера из Settings tab | ❌ **сломан** |
| 2 | `loadTemplateVarDefaults` — `default_value` из `wizard_template.json` | ✅ **работает** |
| 3 | `knownPlaceholderFallback` — hardcoded fallback на v0.8.6 | ✅ работает |

Для свежей установки с дефолтным шаблоном уровень 2 отвечает `"5m"` для `urltest_interval` — sing-box доволен, лаунчер запускается. Уровень 3 (hardcoded) даже не достигается. Кастомизация в Settings tab — теряется молча.

### 3.2. Три бага в `loadStateSettingsVars`
...
Все три бага сразу → `loadStateSettingsVars` молча возвращает пустую карту → override-уровень мёртв.

**Что это значит для пользователя:**

| Сценарий | v0.8.8.1 | Корректно (v0.8.8.3) |
|---|---|---|
| Свежая установка, не трогал URLTest | ✅ template default `5m` | ✅ template default `5m` |
| Выставил `urltest_interval=1m` в Settings tab | ❌ silently игнорируется → `5m` в config | ✅ `1m` в config |
| Сломанный state.json с `@urltest_*` | ✅ template default → sing-box стартует | ✅ то же |
...
## 4. v0.8.8.3 — починка override-ветки + path-инварианты

Цель — НЕ исправление критического бага (это сделал v0.8.8.1), а:

1. **Починить override-уровень** (Settings tab → config.json) для тех ~1% юзеров, которые кастомизируют URLTest.
2. **Консолидировать path-инварианты**, чтобы такая глупость («написал путь из головы, не сверил») технически не прошла бы ревью в будущем.
3. **Canary-тест с реальной фикстурой**, чтобы регрессия типа моих трёх багов всплывала громко.
...
Симуляция сломанного состояния пострадавших юзеров **на v0.8.8.1** (live-инсталл):
...
**Это подтверждает: v0.8.8.1 уже устранял критический баг.** Подстановка `@urltest_*` сработала через template-default уровень (loadTemplateVarDefaults — путь к `bin/wizard_template.json` написан правильно). Override-уровень был мёртв, но в этом сценарии не нужен — template default = `5m` = желаемое значение.
...
| Метрика | Значение |
|---|---|
| Релиз, который устраняет критический баг | **v0.8.8.1** (sing-box check проходит, лаунчер стартует) |
| Релиз, который чинит degraded feature | v0.8.8.3 (Settings tab override доходит до config.json) |
| Опубликованных релизов | 2 (v0.8.8.1, v0.8.8.3); v0.8.8.2 — локальный, не достиг публикации |
| Коммитов | 3 (`5a78d9c` v0.8.8.1, `2363baa` v0.8.8.2 attempt, `6fd8a22` v0.8.8.3 fix) |
| Багов в override-ветке самого hotfix | 3 (путь, формат, ключ) |
