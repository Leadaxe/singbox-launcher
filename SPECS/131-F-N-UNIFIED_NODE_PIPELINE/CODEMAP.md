# CODEMAP — 131-F-N-UNIFIED_NODE_PIPELINE (единый конвейер добавления узла)
...
не на что (CANON §4, severity `error`).
...
приложений, CANON §6). Поле `go` уже несёт якорь `файл:строка` — это готовая
...
Правило CANON §4 и теста `:204`: код, который нигде не ставится, обязан быть
...
CANON §6: «в порядке появления при разборе». `AddWarning`
