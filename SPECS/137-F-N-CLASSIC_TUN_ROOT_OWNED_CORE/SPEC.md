...
ядро ниже lx.11 (или неизвестной версии) `--service=copy` не знает, а его
install переписал бы plist на свой файл — диалог показывает причину и
подсказку сначала обновить ядро (Core → Download v<пин>), без команды и без
Retry, только Close.
...
6. **Бамп `constants.RequiredCoreVersion` до lx.11** — вместе с SPEC 136, при
   релизе (сделан: lx.12). Ядро лаунчера ниже lx.11 команды из диалога не
   получает (SPEC 136 §4.1), и старт с TUN невозможен, пока ядро не
   обновлено: релиз лаунчера с этой задачей без ядра lx.11+ не выпускается.
