...
Обе команды исполняет ядро лаунчера, поэтому они проходят гейт SPEC 136 §4.1:
ядро ниже lx.12 (или неизвестной версии) `--service=copy` не знает или
кладёт копию в раннюю раскладку, а его install переписал бы plist на свой
файл — диалог показывает причину и подсказку сначала обновить ядро (Local
tab → Download/Reinstall v<пин>), без команды и без
Retry, только Close.
...
5. **Windows — следующей задачей.** Лаунчер с `requireAdministrator`
   ...
   Не трогаются (норма 5). Продолжение: SPEC 139 (`asInvoker`, повышение
   только для TUN — для proxy-only дыра закрыта) и SPEC 141 (classic под
   правами исполняет только защищённую копию — закрытие для TUN).
6. **Бамп `constants.RequiredCoreVersion` до lx.11** — вместе с SPEC 136, при
   релизе (сделан: lx.12). Ядро лаунчера ниже lx.12 команды из диалога не
   получает (SPEC 136 §4.1), и старт с TUN невозможен, пока ядро не
   обновлено: релиз лаунчера с этой задачей без ядра lx.12+ не выпускается.
