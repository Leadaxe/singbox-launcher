...
- **143** — фича завершена 26.09.2026 (F-C): TEMPLATE_CANON_WALKER — единый канонический обходчик шаблона во всей сборке (главный конфиг, `on_change.set`, пресеты, DNS-серверы), сплайс массива в ветке условного элемента на один уровень, запрет коллапса `["@name"]`, `options` ортогональны `type` (+ `options_open`, `enum` = `text` + закрытые `options`), `@runtime.*` в позиции значения, коды шаблона в «Итог» сборки (`template_degraded`), снятие зашитых списков имён `isIntCastVar`/`intCastVarNames`; контракт 1.1.68–1.1.71, TASKS_LXBOX §64–§67; **SPECS/143-F-C-TEMPLATE_CANON_WALKER/SPEC.md** (§7 — что повторить LxBox), отчёт — **IMPLEMENTATION_REPORT.md**
