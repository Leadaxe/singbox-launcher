...
- [ ] **Шаблон.** `NormalizeDNSOptions` без склейки: объявления по тегу
...
- [ ] **Состояние.** `DNSServer.Vars`; копии (`CloneDNSServer`,
      ...
      `dns_options.go`.
- [ ] **Нормализация.** `core/state/record_vars.go`: объявления
...
- [ ] **Сборка.** Подстановка тела шаблонного сервера по объявлениям сервера и
...
- [ ] **UI.** Модель `DNSTemplateVars`; загрузка (перенос ДО фильтра сирот),
      ...
      `root_name_refs.go`.
- [ ] **Бэкап.** Опции с объявлениями (UI, debug API, корпус); декодеры 1.0 и
...
- [ ] **Debug API.** `PATCH /state/dns` (вид `vars`, нормализация),
      `PATCH /state/rules` (нормализация), экспорт/импорт с объявлениями, поля
      `record`/`reason` в предупреждениях.
- [ ] **Контракт.** `backup.schema.json` (`dnsServer.vars` обе стороны),
...
- [ ] **Заметки.** `docs/release_notes/1-6-0.md` EN/RU, `CHANGELOG.md`.
- [ ] **Проверки.** Тесты §12.3 (падают на старом коде); стенд livecmp на
