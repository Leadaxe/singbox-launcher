...
**Current state — gap identified during this SPEC work:**

- **Loader** (`core/template/loader.go`): accepts `type:"secret"` for top-level
  `vars[]` without validation error.
- **Substitute walker** (`core/template/substitute.go::replacementForPlaceholder`):
  treats `"secret"` like `"text"` — falls through to default scalar substitution.
  `@proxy_in_password` substitutes correctly as plain text into the emitted
  `users[].password` field.
- **UI Settings tab** (`ui/configurator/tabs/settings_tab.go:184`): hard-coded
  filter:
  ```go
  if vd.Type == "secret" && !clashSecretSecretVar(vd) {
      continue  // SKIP — silently hidden from UI
  }
  ```
  `clashSecretSecretVar` checks `Name == "clash_secret"` specifically. Any OTHER
  secret var (e.g. `proxy_in_password`) is **silently invisible** in Settings.
  User can't enter a value → `proxy_in_password = ""` → `users:[{"":""}]`
  emitted, which sing-box treats as broken auth state.
- **Preset vars** (`core/template/preset_loader.go:362`): `validateVar` allowlist
  is `outbound/dns_server/enum/text/number/bool` — `"secret"` rejected → preset
  skipped with warning. Cannot use `secret` in preset vars at all.

**Workarounds available now:**
- Use `type:"text"` for any non-`clash_secret` "secret-ish" var — works, but
  password is visible in UI (no masking).
- Continue using the special-case `clash_secret` for the one regenerate button.

**Proper fix (separate follow-up, NOT in this SPEC):**
- Extend `settings_tab.go` to render any `type:"secret"` var as a password-field
  (`*` dot-masking), with a show/hide toggle button. ~30 LOC + new
  `buildSettingsSecretRow` helper. Lift the `clashSecretSecretVar`
  special-case (regenerate button) into one branch of the new helper.
- Optionally: add `type:"secret"` to preset-var allowlist with same UI rendering
  in `preset_ref_edit_dialog.go`.
- Document in `TEMPLATE_REFERENCE.md` § var types.

Until that fix lands, `proxy_in_password` should remain `type:"text"` in
`bin/wizard_template.json` — password visible in UI but at least functional.
