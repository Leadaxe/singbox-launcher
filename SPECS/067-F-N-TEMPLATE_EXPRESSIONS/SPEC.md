...
3. **Runtime globals** — namespace **`@runtime.*`** (**`@runtime.platform`** /
   **`@runtime.arch`**) — сразу в predicates (без globals `#if` не закрывает
   platform-условия в payload). Namespace расширяемый — новые поля под `@runtime.`.
...
* **`@`-prefixed:**
  ...
  - **Runtime globals** — namespace `@runtime.*` (`@runtime.platform`,
    `@runtime.arch`) — только в `#if.and`/`or` predicates; inject'ятся walker'ом
    из `runtime.GOOS` / `runtime.GOARCH`; **не** в `vars[]`, **не** substitute из
    state. Имя `runtime` зарезервировано под этот namespace.
...
`@var` в таблице — template var из `vars[]` **или** runtime global `@runtime.platform` /
`@runtime.arch` (см. § Runtime globals). Globals поддерживают только text-predicates
...
* Single-key object с ключом `"@varname"`:
  - Если varname начинается с `runtime.` (`runtime.platform` / `runtime.arch`) → runtime global (text scalar).
  - Иначе — lookup в `vars[]`.
  - Value тоже строка:
...
| `{"@runtime.platform": …}` / `{"@runtime.arch": …}` | runtime global (always text) |
| `{"@var": "literal"}` | text template var |
| `#notEmpty` / `#isEmpty` | text, text_list, bool template var; `@runtime.platform` / `@runtime.arch` |
| `#in` / `#notIn` | text template var; `@runtime.platform` / `@runtime.arch` |
| `#matches` | text template var; `@runtime.platform` / `@runtime.arch` |
| `{"#not": <inner>}` | следует typing inner-predicate'а (negation не меняет тип) |

Mismatch → loader error. `vars[]` с `name` `runtime` → loader error (зарезервированный namespace).
...
### Runtime globals — namespace `@runtime.*` (v1, не откладывать)

Пространство имён `@runtime.*` — pseudo-var, доступные **только** в predicates
`#if.and` / `#if.or` (поля: `@runtime.platform`, `@runtime.arch`; namespace
расширяемый — новые поля добавляются под `@runtime.`). Имя `runtime`
**зарезервировано** под namespace — объявлять `vars[].name == "runtime"` нельзя.
**Ship вместе с walker'ом** — иначе platform-условия внутри `value` снова требуют
дублирования `params[]` / bool-kostyli.

| Global | Runtime source | Scalar (text) | Пример значений |
|---|---|---|---|
| `@runtime.platform` | `runtime.GOOS` | GOOS текущего процесса | `"darwin"`, `"windows"`, `"linux"` |
| `@runtime.arch` | `runtime.GOARCH` | GOARCH текущего процесса | `"amd64"`, `"arm64"`, `"386"` |

**Семантика:** те же predicate-формы, что у text-var — equality, `#in`,
`#notIn`, `#matches`, `#notEmpty` / `#isEmpty`. **Bare** `"@runtime.platform"` /
`"@runtime.arch"` в списке (bool-form) → validation error (не bool).

**Win7** (legacy `windows/386` binary): `{"@runtime.platform": "windows"}` +
`{"@runtime.arch": "386"}` в одном `and` — эквивалент «только win7-сборка». Совпадает
с дисциплиной `platforms: ["windows"]` + отличие по arch, не отдельная метка
`win7` в `@runtime.platform`.

**Case-sensitivity:** `@runtime.platform` / `@runtime.arch` lookup и сравнение значений —
**case-sensitive**, в lower-case (`"darwin"`, `"windows"`, `"linux"`,
`"amd64"`, `"arm64"`, `"386"`) — те же значения что выдаёт `runtime.GOOS` /
`runtime.GOARCH`. Reserved-name check (`vars[].name == "runtime"`)
тоже strict lower-case: `"Runtime"` / `"RUNTIME"` — НЕ reserved (но
...
```json
"and": [
  {"@runtime.platform": {"#in": ["darwin", "linux"]}},
...
**Reserved names:** в `vars[]` **запрещён** `name: "runtime"` — loader error
(collision с runtime-globals namespace `@runtime.*`).

**Outer `if`/`if_or`:** `@runtime.platform` / `@runtime.arch` **не** допускаются — там только
...
Lookup `@runtime.platform` / `@runtime.arch` — до `resolved` map. `SubstituteVarsInJSON` и все
`#if`-handlers — параметры `goos`, `goarch` (callers: `runtime.GOOS`,
`runtime.GOARCH`; тесты — fake).

### `default_value` поддерживает `#if` (runtime-only)

`vars[].default_value` может быть `#if`-выражением — дефолт вычисляется в runtime
по `@runtime.*` globals. Это обобщение per-platform ключей (`win7` / `<goos>` /
`default`): вместо именованных ключей — выразительные условия (`and`/`or`, `#in`,
`#matches`, …).

**Ограничение:** внутри `default_value`-`#if` разрешены **только** `@runtime.*`;
ссылки на другие `vars[]` **запрещены** (loader error). Причина — на этапе resolve
дефолтов остальные vars ещё не разрешены, порядок объявления не гарантирован;
пускать user-var refs → недетерминизм/циклы. Globals (`@runtime.*`) от порядка
резолва не зависят.

**Две формы размещения:**
...
// per-platform: значение платформенного ключа — #if-дерево (можно мешать со строками)
"default_value": {"default": {"#if": {"and": [{"@runtime.platform": "windows"}, {"@runtime.arch": "386"}],
                                      "value": "gvisor", "else": "system"}}}
```

Выбранная ветвь `#if` рекурсивно разрешается до скаляра (строка / число / bool →
строка). Если condition false и нет `else` → пустой дефолт (трактуется как «ключ
не найден», перебор продолжается / падает на `Scalar`).

**Реализация:** `VarDefaultValue.PerPlatform` хранит значения как дерево
(`map[string]interface{}`: строка ИЛИ `#if`-узел); `ForPlatform(goos, goarch)`
вычисляет `#if` через `selectIfBranch` (nil `resolved`-map → user-vars недоступны).
Валидация — `validateDefaultValueIf` с **пустым** `varByName` (любой user-var ref →
«unknown var»). См. `core/template/vars_default.go`, `template_validate.go`.
...
1. **Bare `if[]` / `if_or[]` элементы** — везде добавить `@`-prefix. Loader
   ...
   template: entry `inbounds` has bare var-ref "tun" in if[]; use "@tun" form.
   ```
2. **Var с `name: "runtime"`** в `vars[]` — переименовать (collision с
   runtime-globals namespace `@runtime.*`). Loader error:
   ```
   template: vars[].name "runtime" is reserved (runtime global namespace); rename.
   ```
3. **Case-sensitivity:** strict lower-case для reserved (`runtime`),
   case-sensitive для var-имён везде (consistent с прежним поведением).
   `"Runtime"` (capital R) технически НЕ reserved — но плохая практика,
...
> **Статус (факт):** выполнено. `TEMPLATE_REFERENCE.md`, `WIZARD_TEMPLATE.md`,
> `WIZARD_TEMPLATE_RU.md`, `DATA_FLOW.md` уже содержат разделы про `#if` и
> `@runtime.*`; `WIZARD_TEMPLATE_RU.md` переписан в syntax-reference (коммит
> `4a964b2`). `ARCHITECTURE.md` упоминает `#if` кратко. Таблица ниже — как
> чеклист закрытия (оставлена для истории).

| Файл | Обязательно | Что добавить / исправить |
|---|---|---|
| [`docs/TEMPLATE_REFERENCE.md`](../../docs/TEMPLATE_REFERENCE.md) | **да** | Новый раздел **`#if` construct**: форма, map-spread / array-element, predicates, naming discipline (`#` vs bare vs `@`). Runtime globals `@runtime.platform` / `@runtime.arch`. `@`-only в outer `if`/`if_or`. Таблица «Где лежит реализация»: `substitute.go` — `#if` walker; `template_validate.go` — validate `#if` + outer refs. **Исправить** lifecycle-диаграмму: `LastTemplateLauncherVersion` vs `AppVersion`, не `RequiredTemplateRef`. Cross-ref SPEC 067. |
| [`docs/WIZARD_TEMPLATE.md`](../../docs/WIZARD_TEMPLATE.md) | **да** | Примеры `"if": ["@var"]`; параграф про `#if` для conditional fields; validation: bare `if[]` → error. |
| [`docs/WIZARD_TEMPLATE_RU.md`](../../docs/WIZARD_TEMPLATE_RU.md) | **да** | Переписан в справочник по синтаксису (рус.): top-level, vars, #if, params, presets. |
...
> **Статус реализации (факт):** SPEC реализован (коммиты `9987ca7`,
> `1fb0f76`, `f202123`, `24004ff`, `8d95e5a`, `85266a9`, `4a964b2`,
> `a331b08`). Имена/сигнатуры ниже — **проектные**; фактические имена в коде
> отличаются (см. inline-пометки `Факт:`). В частности `#if`-walker и
> predicate-функции получили суффикс `Ctx`, а общий outer-`if`/`if_or`
> evaluator вынесен в новый файл [`core/template/ifexpr.go`](../../core/template/ifexpr.go)
> (`EvalIf` / `EvalIfWithReason`) — единый источник для build-pipeline и UI.

### `core/template/substitute.go` (EDIT) + `core/template/ifexpr.go` (NEW)

`SubstituteVarsInJSON` приобретает параметры `goos`, `goarch` (для `#if`
predicates `@runtime.platform` / `@runtime.arch` и для `ParamIfSatisfied`, который проверяет
...
> **Факт:** в коде функция называется `handleIfMapSpreadCtx`
> ([substitute.go:245](../../core/template/substitute.go)), array-вариант —
> `handleIfArrayElementCtx` ([:271](../../core/template/substitute.go)).
...
> **Факт:** в коде эта логика разнесена: выбор ветки — `selectIfBranch`
> ([substitute.go:283](../../core/template/substitute.go)), оценка condition —
> `evaluateIfCondition` ([:302](../../core/template/substitute.go)) +
> `evaluatePredicateList` ([:330](../../core/template/substitute.go)); функции
> `evaluateIfBody` нет.

`evaluatePredicate` — switch по формам predicates. Lookup scalar:
1. `@runtime.platform` → `goos`; `@runtime.arch` → `goarch`.
2. Иначе template var из `resolved`.
Реализует type-checked lookup (`#notEmpty` на bool → `scalar=="true"` etc).
Bare `"@runtime.platform"` / `"@runtime.arch"` → false + defensive (validator ловит раньше).
...
* `validateIfConstruct(rawValue json.RawMessage, varByName)` — рекурсивный walk
  ...
  <br>**Факт:** сигнатура — `validateIfConstruct(rawValue json.RawMessage,
  varByName map[string]TemplateVar, context string) error`
  ([template_validate.go:239](../../core/template/template_validate.go));
  predicate-валидаторы — `validateIfPredicate` ([:384](../../core/template/template_validate.go))
  и `validateVarPredicateRHS` ([:434](../../core/template/template_validate.go)).
  - Ровно один из `and`/`or` присутствует и непустой list.
  - `value` field present (не nil).
  - `vars[]` не содержит `name` `runtime` (зарезервированный namespace).
  - Каждый predicate в `and`/`or`:
    - Bare string — template bool var; `"@runtime.platform"` / `"@runtime.arch"` → error.
    - Single-key object — key `@varname`: `runtime.*` (runtime global) → text predicates only;
...
* `validateOuterIfRefs(ifList []string, varByName, context string)` — для outer
  ...
  <br>**Факт:** сигнатура — `validateOuterIfRefs(ctx string, ifNames, ifOrNames
  []string, varByName map[string]TemplateVar) error`
  ([template_validate.go:25](../../core/template/template_validate.go)):
  `ctx` идёт первым, и `if` / `if_or` передаются двумя отдельными списками.

Вызывать из `loader.go` после parse каждой entry, по всем `value`s и `default`s.
**Факт:** провязано в `LoadTemplateData` → `ValidateWizardTemplate`
([loader.go:252](../../core/template/loader.go)).
...
> **Факт:** `proxy_in_password` в коде — `type: "secret"` (не `"text"`):
> follow-up «generic `type:"secret"` rows» в Settings tab был сделан в том же
> цикле (коммит `85266a9`), так что пароль маскируется в UI. См. обновлённый
> статус в § Open questions → `type: "secret"`.
...
> **Факт:** в коде condition строже — `and: ["@proxy_in_auth_enabled",
> {"@proxy_in_username": "#notEmpty"}]`. Доп. predicate `#notEmpty` на
> `proxy_in_username` страхует от broken-state `users:[{"":""}]` (пустой логин
> при включённой auth) — это закрывает строку «broken state» из manual matrix
> ниже не на уровне UI, а прямо в шаблоне.
...
15. `TestIf_Predicate_Platform_In` — `{"@runtime.platform": {"#in": ["darwin","linux"]}}`
    true/false по fake goos.
16. `TestIf_Predicate_Arch_Equality` — `{"@runtime.arch": "386"}` на fake goarch.
17. `TestIf_Predicate_Platform_BareString_LoaderError` — bare `"@runtime.platform"` в
...
23. `TestVars_ReservedName_Runtime_LoaderError` — `vars[]` с `name: "runtime"` → error;
    `TestVars_FormerReservedNames_NowAllowed` — `platform` / `arch` снова допустимы.
...
27. `TestOuterIf_PlatformGlobal_LoaderError` — `"if": ["@runtime.platform"]` → error
...
* Reserved `vars[].name` `runtime` → error (`platform` / `arch` снова допустимы).
* `default_value`-`#if`: `@runtime.*` → ok; ссылка на user-var → error; неизвестный `@runtime.<field>` → error.
...
* **`@runtime.platform` / `@runtime.arch` в placeholder substitution** (вне `#if` predicates) —
  не поддерживаются; `@runtime.platform` в JSON string value substitute'ится как unknown
...
| true | true | пусто | пусто | **`users` НЕ эмитится** — broken-state предотвращён прямо в шаблоне: condition `#if` включает `{"@proxy_in_username": "#notEmpty"}` (см. § wizard_template.json «Факт»). До этого предполагался UI-guard; фактически решено template-side. |

## Phases — implementation order

1. **Walker extension** — `substitute.go`: `#if` + predicates + **`@runtime.platform` /
   `@runtime.arch`** (`goos`, `goarch`); synthetic templates в тестах (в т.ч. platform/arch
   predicates). Без правок `wizard_template.json` на этом шаге.
2. **Validator** — `template_validate.go` + validation тесты (в т.ч. reserved
   `platform`/`arch`, bare `"@runtime.platform"` в `and[]`).
...
6. **Документация** — по таблице § «Документация» (минимум:
   `TEMPLATE_REFERENCE.md`, `WIZARD_TEMPLATE*.md`, `ARCHITECTURE.md`,
   `DATA_FLOW.md`).
...
9. **Phase 9 — Preset.Rule → Preset.Rules schema rename (post-Phase-8 amendment).**
   Rename `Preset.Rule map[string]interface{}` → `Preset.Rules []map[string]interface{}`
   (JSON key `"rule"` → `"rules"`). Hard cutover, no backward compatibility
   (custom-template authors migrate `"rule": {...}` → `"rules": [{...}]`).
   Unblocks multi-rule presets like `split-all-traffic` (previously broken
   because two-rule structure couldn't fit single-map schema → silently dropped
   by loader). `ExpandPreset` § 4 loops over `preset.Rules`;
   `PresetFragments.RoutingRule` → `RoutingRules []map`; consumers
   (`preset_merge.go`, `resolve_route.go`, `rules_pipeline.go`) and UI
   `preset_ref_convert.go` (multi-rule label "[1] / [2] / …") +
   `preset_ref_edit_dialog.go` (preview emits `rule` for single / `rules` for
   multi). Touches 14 preset entries in `bin/wizard_template.json` (13
   single-rule wrapped in 1-element array; 1 already-array `split-all-traffic`
   just gets the key rename). 34 test fixtures updated. `dns_rule` (singular)
   NOT touched — separate schema, separate decision if/when it ever needs
   multi-rule support.
...
`value`). Кандидаты для схлопывания через `#if` + `@runtime.platform`:
...
* `presets[].id="split-all-traffic-mac-win"` + `split-all-traffic-linux` —
  один preset с `source_port_range` через `#if` на `@runtime.platform`.
...
### `type: "secret"` UI rendering (template var type)

The template `vars[].type` enum currently includes `"secret"` (used historically for
`clash_secret` — Settings tab renders a special single-line entry + "Regenerate"
button via `clashSecretSecretVar`). New use case: `proxy_in_password` (template
already declares it during proxy-in auth work) — would benefit from
password-style masking (dot-substitution) in UI.

> **Статус (факт):** частично сделано. Settings-tab часть закрыта в коммите
> `85266a9` (generic `type:"secret"` rows); preset-уровень — **всё ещё открыт**.
> Подробности ниже обновлены под фактическое состояние кода.

**Что сделано (top-level `vars[]` / Settings tab):**

- **Loader** (`core/template/loader.go`): принимает `type:"secret"` для top-level
  `vars[]` без validation error.
- **Substitute walker** (`core/template/substitute.go`): трактует `"secret"`
  как `"text"` — обычная скалярная подстановка. `@proxy_in_password`
  подставляется как plain text в `users[].password`.
- **UI Settings tab** (`ui/configurator/tabs/settings_tab.go`): хардкод-фильтр
  `clashSecretSecretVar` **удалён**. Любой `type:"secret"` var теперь рендерится
  generic-хелпером `buildSettingsSecretRow`
  ([settings_tab.go:222,396](../../ui/configurator/tabs/settings_tab.go)):
  `widget.NewPasswordEntry()` (dot-masking + встроенный show/hide eye) +
  Regenerate-кнопка (бывший clash_secret-кейс свёрнут в одну ветку хелпера).
  Поэтому `proxy_in_password` объявлен как `type:"secret"` (не `"text"`) и
  виден/маскируется в Settings.

**Что ещё открыто (preset-уровень):**

- **Preset vars** (`core/template/preset_loader.go`): `validateVar` allowlist —
  `outbound/dns_server/enum/text/number/bool`; `"secret"` **по-прежнему
  отвергается** → preset со `secret`-var skip'нется с warning. Использовать
  `secret` в preset-vars нельзя.
- **Preset var editor** (`ui/configurator/tabs/preset_ref_edit_dialog.go`): нет
  ветки `case "secret":` — попадает в `default` и рендерится обычным
  `widget.NewEntry()` без маскирования / regenerate.

**Proper fix остатка (separate follow-up, NOT in this SPEC):**
- Добавить `type:"secret"` в preset-var allowlist (`preset_loader.go`).
- Добавить `case "secret":` в `preset_ref_edit_dialog.go` с password-field +
  regenerate (зеркалит Settings-tab).
- Document in `TEMPLATE_REFERENCE.md` § var types.
