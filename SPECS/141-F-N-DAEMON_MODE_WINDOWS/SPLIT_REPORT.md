...
| `core/daemon_service_state.go` (было `_darwin.go`) | daemon-платформы | состояния, `NeedsInstall`/`NeedsBootstrap`/`CopyUsable`, гейт и разбор версий ядра, обход цепочки `checkRootOwnedChain`, сверка sha, `compareDaemonServiceProcess`, сайдкар-версия, кэши sha и версии (`fileHashKey` — данные, заполняет платформа), `shortSHA` |
| `core/daemon_service_state_darwin.go` (новый) | darwin | раскладка `/Library/…`, legacy lx.11, `inspectDaemonServiceDefinition` (plist), звено цепочки `checkRootOwnedEntry` (`Stat_t`), `statHashKey` (dev, inode), `launchctl print`, `compareDaemonServiceRunning` |
| `core/daemon_service_state_windows.go` (новый) | windows && !386 | заглушки: раскладка пуста, служба «не установлена», звено и ключ файла — ошибка, состояние SCM — пусто |
