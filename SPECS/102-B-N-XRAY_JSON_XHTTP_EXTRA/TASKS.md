...
- [ ] 4.2 Живая проверка: три узла через `URLTestOutbound` — до и после фикса,
      результат в IMPLEMENTATION_REPORT.
- [x] 4.3 `docs/release_notes/upcoming.md` (EN → Highlights, RU → Основное).
- [x] 4.4 Заполнить `IMPLEMENTATION_REPORT.md`.
- [ ] 4.5 Переименовать папку `102-B-N-…` → `102-B-C-…` после приёмки.
