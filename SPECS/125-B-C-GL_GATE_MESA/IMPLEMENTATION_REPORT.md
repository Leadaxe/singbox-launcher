...
## Полевая валидация RC2 `v1.5.5-24-ga6e0b5a2-prerelease` (13.09.2026, GeForce GT 440, Win10 19045)

Отчёт пользователя и три скриншота:

- Старт на железе — окно, без диалогов.
- Diagnostics → «Включить Mesa3D» → подтверждение «Лаунчер сейчас перезапустится» → самоперезапуск → окно **под Mesa** (llvmpipe). Падений нет — d3d12 был единственной причиной (§6 F2), пин при каждом старте (R1) её снял.
- Диалог возврата §2.5 показал **настоящий** renderer `GeForce GT 440/PCIe/SSE2` — обход Mesa временным rename (R3) работает; RC1 в этом месте показывал `D3D12 (…)`.
- «Да» → самоперезапуск → окно на железе. «Отключить Mesa3D» из Diagnostics — то же, в обратную сторону. Переключения туда/обратно «без сообщений».
- Остатки `.off` от RC1 кнопка распознала как «Mesa отключена» и предложила включить; после их удаления включение прошло копией из `mesa3d/`. Оба пути `EnableMesa` пройдены.

Критерии приёмки 1–14 подтверждены на стенде, кроме 7 (`MESA_DEBUG=1` не запрашивался) и 9 (`-tray` не проверялся — ветка без диалогов, покрыта тестами `decideGate`).
