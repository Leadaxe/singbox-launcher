...
**Состояние: ЗАКРЫТО.** Боевой путь переключён, рукописных эмиттеров не
осталось.
...
- **боевой путь переключён**: `ShareURIFromOutbound` выбирает секцию по типу
  тела через реестр (`SchemeForSingboxType`) — диспетчера по имени схемы в
  коде не осталось; сняты 15 файлов `shareuri_*.go` (~1700 строк) и две
  осиротевшие функции (`appendEarlyDataToPath`, `NormalizeRealityKeyShare`);
- **генератор снимка удалён вместе с эмиттерами**: снимок стал ЗАМОРОЖЕННЫМ
  эталоном, пересъёмка означала бы сверку движка с самим собой.

**Три свойства, которые держал код и пришлось объявить данными** — их нашла
не сверка, а падение старых тестов эмиттеров:

1. `detour` — **эмит-онли** (`round_trip_only: "emit"`, `dialer#uri`). Поле
   `managed`: его пишет сборка конфига, санитайзер снимает из тела узла, но
   ссылка на узел внутри цепочки обязана нести имя следующего хопа. Читать
   обратно нельзя — тег чужого конфига у нас не существует.
2. **Отказ при нескольких пирах** (`emit.refuse_when` у wireguard): ссылка
   выражает ОДНОГО пира, и отдать первого молча значит выдать за узел
   половину маршрута.
3. Списки awg-полей в тестах — теперь из `kind_when` реестра, а не из
   таблицы в коде: таблица жила рядом с эмиттером и пережила бы его копией.

Остаток:

- `emit.omit_port` реализован, но не выставлен нигде — ждёт ответа LxBox
  (`TASKS_LXBOX.md` §33.6).
...
# Состояние и следующий шаг (передача, 19.09.2026 — виды источника)

Предыдущие передачи — в истории выше.

## Что готово этой волной

- **`contract/registry/source_kinds.json`** — таблица видов источника,
  двенадцать веток (контракт 1.1.41). Термин владельца «вид источника», ключ
  записи `source_kind`; имя `documents.json` из черновика LxBox и слово
  «документ» в норме не приняты. Сведение имён — `TASKS_LXBOX.md` §37.1.
- **Движок уровня вида** — `core/config/linkmap/source.go`: `ClassifySource`
  исполняет `detect` → `unwrap` → `redetect` → `elements`. Распаковщики
  (`unwrap`) передаются ИЗВНЕ по имени: формат контейнера предикатом не
  выражается, но вызывается он объявленным именем, а не веткой в сниффере.
- **Новые примитивы `detect`** — `first_section_fold`, `prefix_trim`,
  `min_len`, `requires_after_unwrap`, `max_unwrap_depth`; `array_elem_any_keys`
  научился НЕСКОЛЬКИМ уровням `[]` (`[].outbounds[].protocol`).
- **Временный признак «`://` в тексте» снят** с `linkmap/parse.go`: оболочку
  снимает тот, у кого декодер объявлен с областью (`scope`) — признак
  структурный, из формы, а не из входа.
- **Раннеры** — `TestSourceKinds` (18 кейсов «текст → вид → число элементов»)
  и `TestSourceKindsTableWellFormed` (уникальность `priority`, единственность
  `default`, у каждого вида есть чем разобрать элемент). Линтер схемы
  (`TestContractMapperSectionsMatchSchema`) научился судить `source_kinds`.
- **Дельты** `D133-35`…`D133-38`, **странности** `Q133-68`, `Q133-69`.

## Что НЕ сделано — остаток

1. **`body_classify.go` с боевого пути не снят.** Таблица готова и покрыта
   раннером, но переключение — отдельный шаг: пять боевых вызовов
   (`parse_body.go:197`, `source_loader.go:325`, `add_server_dialog.go:557`
   и `:670`, `sources_json.go:38`) и около тринадцати тестовых файлов.
   Вместе с переключением решается судьба `BodyKind` как ПУБЛИЧНОГО типа: на
   него смотрит UI (`sources_json.go`, `add_server_dialog.go`), и заменять его
   именем вида (строкой) надо во всех точках разом, иначе появится второй
   источник правды — ровно то, от чего затеяна кампания.
2. **Распаковщики не подключены к боевому пути.** `ClassifySource` принимает
   их параметром, но карта `{"base64_utf8": …, "amnezia_vpn": …}` сегодня
   живёт только в раннере. Боевые тела — `DecodeSubscriptionContent`
   (`subscription/decoder.go`) и распаковщик Amnezia
   (`node_parser_amnezia.go:96-360`); оба остаются на месте по границе
   `SCHEMES.md` §13.5, их надо лишь ЗАРЕГИСТРИРОВАТЬ под объявленными именами.
3. **`on_unrecognized` не объявлен** намеренно: ветка `uri_lines` стоит
   `default: true` и ловит всё остальное, поэтому «не опознано» недостижимо.
   Объявить придётся ровно тогда, когда default-ветка уйдёт.
4. **`xray_balancer.go` оценён, не переделан** (как и просило ТЗ):
   `balancers[0]` → группа — связь между узлами, а не свойство элемента;
   таблицей вида источника не выражается, границу см. `SCHEMES.md` §13.5.
5. **`xray_json_array.go` НЕ удалён** — осознанно, `QUIRKS.md` Q133-69:
   из 901 строки нарезкой занята одна функция на 15 строк, остальное — сборка
   ДОКУМЕНТА (владение, дедуп, `dialerProxy` → цепочки, теги).
6. **`on_invalid: {action: "default_from"}` движком НЕ ИСПОЛНЯЕТСЯ**
   (найдено ревизией зеркала, контракт 1.1.53). Действия `default_from` в
   `exec.go` нет вовсе: `applyMissing` откатывается на `default_from` только
   когда источник ПРОМОЛЧАЛ либо дал пустую строку, а present-значение
   никакая ветка не судит. Значит вторая половина правила
   `sni_heuristic_falls_back_to_server` («имя без точки и двоеточия уступает
   адресу сервера») объявлена, но не работает — и у hysteria2, который несёт
   её с контракта 1.1.16, и у anytls, которому она добавлена в 1.1.53.
   Проверено прогоном: `sni=Germany` доезжает в `tls.server_name` как есть
   у обеих схем. Кейсы корпуса `uri/anytls/sni_label_falls_back_to_server`
   закрепляют СЕГОДНЯШНЕЕ поведение и станут красными, когда примитив
   появится, — тогда ожидание правится прогоном `-update`.
   Отдельного кейса у hysteria2 нет намеренно: правило одно, и второй
   кейс на тот же неисполняемый примитив ничего бы не добавил.
