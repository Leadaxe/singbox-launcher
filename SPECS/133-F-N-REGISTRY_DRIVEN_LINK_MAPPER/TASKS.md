...
## W0 — грамматика, линтеры, движок (parse) — 16 ч

Без перевода схем: движок появляется, но ни одна схема на него ещё не идёт.

- [ ] `schema/registry_uri.schema.json` — JSON-схема секции `uri` в новой
      грамматике (13 групп примитивов, `SCHEMES.md` §12); валидирует обе стороны
...
- [ ] Линтеры реестра (`SPEC.md` §7) — один Go-тест, 7 проверок
- [ ] Прогон корпуса с флагом «не заменять `+`» → список кейсов с изменённым
      телом (`SPEC.md` §9.3); **результат — в отчёт владельцу, решение не принимать**
...
Если владелец подтвердит объём (`SPEC.md` §11).

- [ ] Форма без схемы, `space: json`, таблица `json.<path>`
- [ ] `xray_outbound_convert.go`/`xray_protocols.go`/`xray_hysteria.go` удалены
- [ ] `xray_json_array.go`/`xray_balancer.go` **остаются** — это сборка
      документа, не перевод узла

## W9 — зачистка и документация — 8 ч

- [ ] Ни одной схемной функции в пакете маппера (критерий конца кампании)
- [ ] Греп-страж — по ответу на вопрос 4
- [ ] `gendocs`: страницы протоколов показывают, что читает общий цикл и
      каким примитивом выражено каждое структурное место
- [ ] `contract/docs/CANON.md` — норма `SPEC.md` §10; `VERSION` bump
- [ ] `TASKS_LXBOX.md` — норма и сверка с их фичой 480
...
|---|---|---|
| W0 грамматика + линтеры + движок | 16 | нет |
...
| W8 xray | 12 | после W7 |
| W9 зачистка и доки | 8 | нет |
| **Итого** | **126 ч** | |

Критический путь при полном параллелизме W2–W6: 16+6+8+14+12+12+8 = **76 ч**.
