...
**Исполняется с контракта 1.1.48** (TASKS_LXBOX §44.1): атрибут `comment` у
...
| `emit.json_always` | ключи, которые клиенты ждут даже пустыми. **Уточнено 1.1.48 (норма с 1.1.36, TASKS_LXBOX §33.2): КАРТА «ключ → заполнитель», а не список имён** — заполнитель у ключей разный (`v: "2"`, `aid: 0`, `net: "tcp"`, `host: ""`), и списком его не выразить |
| `emit.refuse_when` | **ДОБАВЛЕНИЕ (описано здесь с 1.1.48, в схеме — с 1.1.36):** список `{path, len_gt, why}` — условия, при которых узел ссылкой НЕ выражается: массив по `path` длиннее `len_gt` → отказ с текстом `why`, а не ссылка по первому элементу. «Сколько сущностей влезает в ссылку» — свойство ФОРМАТА схемы; у wireguard `peers` > 1 |
...
### 0.12b. Сверка оверлеев эмита (контракт 1.1.48, TASKS_LXBOX §42/§44.3)
