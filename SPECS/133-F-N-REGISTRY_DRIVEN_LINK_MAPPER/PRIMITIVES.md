...
| `emit_normalize` | **DRAFT** | обращение `normalize` на выходе; сегодня одно правило — `port_range_spec_uri` |
| `emit_value_map` | **FROZEN** | ЯВНАЯ обратная таблица «значение тела → написание ссылки» для НЕИНЪЕКТИВНОЙ `value_map` |
| `round_trip` / `round_trip_why` | **FROZEN** | объявленный ОТКАЗ от обратного хода; причина обязательна |
| `compose` объектом | **FROZEN** | `{template, from[], omit_when_empty[]}`; строка — короткое написание, пути берутся из самого шаблона |
...
| эмит: `param_order` = **алфавит** (правило, не перечень); перечень — только там, где порядок обязан отличаться, и он же задаёт порядок ключей формы-КОНТЕЙНЕРА | **FROZEN** |
| эмит: пробел на выходе = `%20` | **FROZEN** |
| эмит: каноническое имя параметра = **ПЕРВЫЙ ИСТОЧНИК записи** (`query.<имя>`), а НЕ первое в `aliases` | **FROZEN** (уточнено 19.09.2026, см. ниже) |
| эмит: ветка `sets` под ПУСТЫМ ключом обратным ходом не выбирается никогда | **FROZEN** |
| эмит: `decode_extra.passes` действует и на выходе — кодировать `passes + 1` раз | **FROZEN** |

**Уточнение «каноническое имя» (19.09.2026).** Прежняя формулировка «первое в
`aliases`» на живых данных неверна и меняет вид ссылки: `aliases` перечисляет
АЛЬТЕРНАТИВНЫЕ написания ЧТЕНИЯ, а канон стоит в `source`. У `tcp_keep_alive`
алиас `tcpKeepAlive`, у http-`headers` — `extra-headers`; взяв первое из
`aliases`, ссылка получает имя чужого диалекта. Это не переименование
атрибута, а исправление прозы о нём — правило §0 соблюдено.

### 0.6b. НОРМА ВИДА ссылки (решение владельца 19.09.2026)

**Вид ссылки = де-факто формат схемы для ЧУЖИХ клиентов, а не симметрия
таблицы.** Copy link существует для обмена; «наш разбор восстановит сам»
верно только для НАШЕГО разбора.

| Атрибут секции | Статус | Значение |
|---|---|---|
| `emit.names` | **FROZEN** | переименование параметра НА ВЫХОДЕ: канон разбора → написание ссылки. Имя флага insecure — свойство СХЕМЫ (`allowInsecure` / `allow_insecure` / `insecure`), общий блок выбрать за неё не может. Действует и по форме-контейнеру |
| `emit.json_map` | **FROZEN** | форма-КОНТЕЙНЕР: «ключ → путь тела \| `$param.<запись>` \| `$label` \| `=литерал`» |
| `emit.json_always` | **FROZEN** | ключи контейнера, которые клиенты ждут ДАЖЕ ПУСТЫМИ. **КАРТА «ключ → заполнитель»**, а не список имён: заполнитель у ключей разный (`v: "2"`, `aid: 0`, `net: "tcp"`, `host: ""`) |
| `emit.userinfo{form,padding}` | **FROZEN** | выходная форма userinfo; `padding` объявлен явно, потому что Go пишет `=`, Dart срезает |
| `emit.userinfo.empty_separator` | **FROZEN** | «@» у узла БЕЗ userinfo (`hy2://@host`) |
| `emit.userinfo.keep_empty_tail` | **FROZEN** | разделитель у ПУСТОГО хвостового компонента (`socks4://userid:@host`) |
| `emit.omit_port` | **FROZEN** | порт опускается при совпадении с `defaults`; обращать `defaults.server_port` напрямую нельзя |
| `emit.form_from` | **FROZEN** (был DRAFT) | написание схемы по телу либо по роду (`$kind`) |
| `userinfo.emit` | **FROZEN** | `false` — userinfo не пишется: канон схемы кладёт то же значение в query, и запись в оба места отдала бы секрет дважды |
| `emit.omit_default` с двоеточием (`fp:random`) | **FROZEN** | сужение до ОДНОГО значения: «не писать параметр» и «не писать его при дефолте» — разные правила |

### 0.12a. `emit_as` — сериализация значения

| Значение | Что делает |
|---|---|
| `join` | список через `list.sep` (`alpn`: тело — массив, ссылка — строка) |
| `bool01` | булев пишется `1` (`insecure=1`) |
| `pairs` | карта тела в строку пар; разделитель пар — `list.sep`, внутри пары — `emit_pair_sep` |
| `json` | вложенная структура ключом JSON-формы |
| `raw` | как есть; булев — СЛОВОМ, обращение `value_map` НЕ применяется |

**Угадывать по типу нельзя.** Булев `true` у одной схемы пишется `1`
(`insecure`), у другой словом (`noGRPCHeader` у Xray) — это свойство
ДИАЛЕКТА, а не типа значения. `raw` нужен и там, где два написания входа
читаются в одно значение, а на выходе канон один (`h2` и `http` у
контейнера): обращение таблицы вернуло бы устаревшее написание.

**УМОЛЧАНИЕ БЕЗ `emit_as` — БУЛЕВ ПИШЕТСЯ СЛОВОМ** (`true`/`false`), а не
цифрой. **FROZEN, контракт 1.1.53** (ревизия зеркала LxBox 24.09.2026, §3
п.3): Go писал словом, Dart — цифрой, и расхождение компенсировалось
оверлеем `emit_as: raw` у трёх полей xhttp (`xmux.hMaxRequestTimes` и
родня). Оверлей снимается, умолчание объявлено здесь.

Словом, а не цифрой, потому что `bool01` — это ИМЕНОВАННЫЙ диалект
(`insecure=1`), и он объявляется явно там, где нужен; умолчание обязано
быть тем написанием, которое несёт БОЛЬШЕ смысла в чужих руках и не
теряет тип на круге. `1`/`0` неотличимы от числового поля, а `true`/`false`
читаются обратно однозначно — и ровно так пишет сам Xray те свои поля, у
которых написание не объявлено отдельно.

Проверяется кейсом корпуса `uri/vless/xhttp_bool_default_spelled`: три
булевых поля xhttp БЕЗ `emit_as` обязаны уехать в ссылку словом.
...
**Исполняется с контракта 1.1.49** (TASKS_LXBOX §45.1): атрибут `comment` у
`label`, выставлен у обеих секций wireguard, читающих `ini.$comment.Peer`
(`conf` и форма `conf_b64` у `uri`). Своего умолчания у движка нет — без
атрибута годится первый непустой комментарий как есть.
...
| `emit.json_always` | ключи, которые клиенты ждут даже пустыми. **Уточнено 1.1.49 (норма с 1.1.36, TASKS_LXBOX §33.2): КАРТА «ключ → заполнитель», а не список имён** — заполнитель у ключей разный (`v: "2"`, `aid: 0`, `net: "tcp"`, `host: ""`), и списком его не выразить |
| `emit.refuse_when` | **ДОБАВЛЕНИЕ (описано здесь с 1.1.49, в схеме — с 1.1.36):** список `{path, len_gt, why}` — условия, при которых узел ссылкой НЕ выражается: массив по `path` длиннее `len_gt` → отказ с текстом `why`, а не ссылка по первому элементу. «Сколько сущностей влезает в ссылку» — свойство ФОРМАТА схемы; у wireguard `peers` > 1 |
...
### 0.12b. Сверка оверлеев эмита (контракт 1.1.49, TASKS_LXBOX §42/§45.3)
