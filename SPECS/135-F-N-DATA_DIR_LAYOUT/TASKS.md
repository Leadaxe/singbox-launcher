...
- [x] Чекбокс с условиями недоступности (§4.2), скрыт на macOS.
- [x] `core/storage_switch.go`: включить/выключить по §4.2 (копия → маркер → удаление старого → `RestartSelf`), остаток при отказе удаления → `bin.moved-<дата>`.
- [x] Интеграционный тест переключения (§8).
