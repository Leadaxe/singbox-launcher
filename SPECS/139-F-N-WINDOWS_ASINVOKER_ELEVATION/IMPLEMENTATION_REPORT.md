...
## Принятый риск до SPEC 141

Записан в SPEC §10: принятый риск, окно до выхода 141. Сценарий §11 п. 9
(обычный пользователь, пароль вводит администратор) исполняет
`<Data>\bin\sing-box.exe` из каталога, в который пишет обычный пользователь,
под учётной записью администратора — дыра SPEC 137 §8 п. 5. Закрывается
SPEC 141 §8 (classic под правами исполняет только копию из Program Files); код
139 не меняется.
