...
| раздел «Уведомления» | см. лог | Дизайн владельца: `nodewarn.Section` из плоского списка стал разделом с шапкой-счётчиками (`✖ N · ⚠ N · ⓘ N`, нулевые уровни молчат), тремя подразделами Errors → Warnings → Info своего цвета и `widget.Accordion` (`MultiOpen`, всё свёрнуто) — по строке на код; внутри путь поля, «Что случилось», «Почему так бывает», «Что можно сделать», «Подробности». Переехал ПОСЛЕДНИМ блоком: вкладка Settings окна источника и вкладка «Подробности» окна Info (там же ветка `node == nil`). Ловушка ширины: заголовок `AccordionItem` — это `widget.Button` без `Wrapping`/`Truncation`, а `accordionRenderer.MinSize` берёт максимум по заголовкам И по `Detail` — заголовок режется по рунам (64), `path` уехал внутрь под `TextWrapWord`; замер: 355pt при трёх кодах и 285pt при пути в 460 символов, окно 620pt. Новые ключи `Notifications`/`Errors`/`What happened:` в `ru.json`, `Warnings`/`For your information`/`Details`/`Why it happens:`/`What you can do:` переиспользованы; l10n_check без новых находок. `TASKS_LXBOX.md` §24.11 |
