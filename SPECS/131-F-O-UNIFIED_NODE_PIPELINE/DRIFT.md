...
### 7.20 🔴 xmux: конвейер портил 13 живых узлов (решено 18.09.2026)

Найдено прогоном настоящего state (186 узлов) уже ПОСЛЕ W2d: у 13 vless +
xhttp узлов одной подписки санитайзер переписывал рабочую секцию
`transport.xmux`. Не расхождение Go/Dart, а **общий дефект обеих сторон** —
правила читаются из реестра, и LxBox реализует их так же.
...
Два независимых дефекта, оба ухудшали живой узел:

1. **`conflicts` судил по НАЛИЧИЮ ключа.** Ядро
   (`transport/v2rayxhttp/xmux.go`) считает `max_concurrency` ↔
   `max_connections` конфликтом, только когда **оба > 0**; `"0"` означает
   «не задано». Санитайзер видел ключ `max_connections` и снимал
   `max_concurrency:"16-32"`, после чего поле возвращалось дефолтом
   `"1-1"` — пропускная способность узла падала в 16–32 раза молча.
2. **`all_or_nothing` был понят наоборот.** Ядро при частично заданной
   секции оставляет незаданные поля **нулями** (= без лимита), а
   санитайзер дописывал дефолты соседей (`h_max_request_times:"600-900"`
   у 7 узлов) с кодом `partial_object_defaulted` — то есть навязывал
   лимиты, которых у узла не было.

**Решение (реализовано).**

* «Не задано» для слоя связей (`conflicts`/`requires`/`forbidden_when`) —
  это отсутствие ключа, `nil`, `""`, `false`, `0`, пустой объект/массив
  **и число-строка из одних нулей** (`"0"`, `"0-0"`: диапазоны XmuxRange
  приходят строками). Правило общее — тем же порядком судятся
  `certificate` ↔ `pins` и `reality` ↔ `ech`; существующие кейсы корпуса
  по этим связям остались зелёными без правок.
  `isEmptyValue`/`isZeroNumericString`, `core/config/nodeflow/sanitize.go`.
* `all_or_nothing` **не влечёт действия санитайзера** и остаётся в реестре
  только как документация о поведении ядра. Код
  `partial_object_defaulted` снят из `registry/warnings.json`, из
  продьюсера и из списка встроенных кодов линтера.
* Кейс корпуса `body/singbox/vless_xhttp_xmux_zero_neighbours` — реальная
  секция (значения анонимизированы) проходит **без изменений и без
  warnings**. Обратный полюс (оба значения заданы → конфликт настоящий,
  младшее по `order` поле снимается) закреплён юнитом в `nodeflow_test.go`.

**Проверка.** Повторный прогон того же state: тело не переписывается ни у
одного узла, кодов `field_conflict`/`partial_object_defaulted` — 0.

Встречная задача LxBox — `contract/TASKS_LXBOX.md` §24.9.

### 7.21 gRPC `service_name`: нормализация снята ядром (18.09.2026)

Обратный ход к фиксу issue #130 (лаунчер 09787fbb, правило реестра
`normalize: grpc_service_name`). Правило **снято целиком**: значение едет в
конфиг как есть в любой форме.

Причина — не ошибка правила, а то, что его предмет переехал в ядро. Вышло
`sing-box-lx` **1.14.1-lx.8** (SPEC 093 ядра): ведущий «/» в `service_name`
теперь разбирается как **custom path Xray** — путь уходит на провод готовым,
с **посегментным** экранированием (`/a/b/Tun` → `/a/b/Tun`, `/a/Stream` →
`/a/Stream`), хвост `|…` отбрасывается. Без ведущего «/» всё по-старому:
имя сервиса, ядро дописывает `/Tun`, имя экранируется одним сегментом
(`a/b` → `/a%2Fb/Tun`).

Тем самым закрыто и ограничение из §4 отчёта по ISSUE_130 (`PathEscape`
рубил многосегментные имена), ради которого перевод только и существовал в
урезанном виде — он чинил `/abcde/Tun` и не мог починить
`/abcde/something/Tun`. После lx.8 перевод стал бы **вредным**: он снял бы
«/» там, где ядро ждёт готовый путь, и восстановил бы 404 на узлах, которые
ядро теперь обслуживает правильно.

Удалено: `normalize: grpc_service_name` в `registry/transports.json`,
значение из enum `normalize` в `schema/registry_body.schema.json`, из
линтера `registryNormalizeModes`, функция `normalizeGRPCServiceName` в
`core/config/nodeflow/coerce.go` и её юниты. `desc_en`/`desc_ru`/`impl` у
поля переписаны под поведение lx.8.

**Гейт `min_core` не вводится** (решение владельца): обе стороны пинят lx.8;
на старом ядре такое значение и без гейта не работало — гейт добавил бы
переписывание там, где норма «как есть».

Расхождения Go/Dart здесь нет и не было: правило читалось из реестра, LxBox
снимает его тем же порядком (`TASKS_LXBOX.md` §24.12, их план §468).

---

### 7.22 QUIC-срез uTLS/REALITY: из прозы в правило реестра (18.09.2026)

Правило «на QUIC-протоколах uTLS и REALITY вычищаются целиком» было
последним крупным правилом ЗНАЧЕНИЯ, которое реестр только описывал прозой
(`tls.json` `policy.quic_strip`), а исполняли частные ветки кода. Переведено
в правило: `forbidden_for` у `body.fields.utls` и `body.fields.reality` +
новый атрибут `forbidden_codes`.

**Найдено при переводе — две вещи, которых проза не называла.**

1. **`hysteria` v1 была в коде, но не в прозе.** Частный набор
   `quicOutboundTypes` включал четыре типа, а `policy.quic_strip` перечисляла
   три (`hysteria2`, `tuic`, `masque`). Проверено по ядру: v1 ходит тем же
   `qtls.Dial` (`sing-quic@v0.7.0 hysteria/client.go:264`), то есть её TLS
   строит `aTLS.Config.STDConfig()`, а он у `UTLSClientConfig` и
   `RealityClientConfig` возвращает ошибку. Разницы между четырьмя нет; в
   реестре теперь названы все четыре.

2. **Срез был МОЛЧАЛИВЫМ на ОБОИХ входах, и по разным причинам.** Тело
   sing-box проходило через `singbox_sanitize.go` и теряло блоки в
   `debuglog`; ссылка теряла их раньше — URI-парсеры `hysteria`/`hysteria2`/
   `tuic` просто не читали `fp` («uTLS на QUIC не читается», D-033). Оба
   входа давали одно тело и НИ ОДНОГО кода: пользователь, у которого
   подписка выдала `fp=` на hysteria2-узле, не узнавал, что параметр не
   сработал. Это не расхождение Go/Dart (Dart молчит так же, §282), а общий
   пробел обеих сторон.

**Почему понадобился новый код, а не общий `tls_field_unsupported_naive`.**
Запрет один, а исход разный. У naive снятое TLS-поле — настройка, которой
узел лишился, и `severity: warning` там уместен. На QUIC uTLS и REALITY не
применились бы в принципе: снята бессмыслица, узел ничего не потерял, и
показывать ему `warning` значит объявить рабочий узел сломанным — та же
ошибка, что исправлял контракт 1.1.2 у `reality_fp_not_chrome`. Поэтому
новый `tls_not_applicable_quic` с `severity: info`.

**Минимальное расширение схемы — `forbidden_codes`.** Атрибут `code` у поля
один, а нужны два кода на один `forbidden_for`. Словарь «схема → код»
переопределяет общий код для отдельной схемы; схема без записи берёт общий.
Альтернатива — разводить `utls`/`reality` на два описания поля с разными
`code` — означала бы дубль всей суб-схемы TLS ради одного атрибута.

**Один код на объект.** Снимается блок целиком, поэтому `key_share` и
`short_id` своих кодов не дают: снят не они, а весь REALITY. То же правило,
что у мусорного `pbk`.

**Побочно исправлена документация.** Генератор печатал правила значения у
полей, запрещённых схеме страницы: `naive.md` обещала `fp` и `pbk`
работающими, `hysteria2.md` — `fp`. Теперь у такого поля печатается строка
«Not applicable to `<scheme>`: removed → код», а поля внутри запрещённого
блока своих правил не показывают вовсе. Запрет ищется по всему пути, а не
только у самого поля: `fp` целится в `tls.utls.fingerprint`, а
`forbidden_for` стоит на `tls.utls`.

---

### 7.23 Потолок MTU у AmneziaWG: из кода парсера в правило реестра (18.09.2026)

Находка №5 `LEGACY_AUDIT` закрыта. Правило «у AmneziaWG-узла MTU выше 1280
заменяется на 1280» жило кодом в парсере ССЫЛКИ (`awgMaxMTU`,
`node_parser_wireguard.go`), и из этого следовали три беды разом.

1. **Правило не применялось к телу из sing-box-импорта.** Парсер ссылки —
   не единственный вход: endpoint, приехавший JSON-ом, клампа не видел
   вовсе. Один и тот же узел ссылкой и объектом получал РАЗНЫЙ MTU, и
   объектная половина тихо уносила в ядро значение, на котором туннель
   поднимается и не несёт данных.
2. **Срез был молчаливым** (`DebugLog`): человек не узнавал, что его 1420
   стали 1280.
3. **Комментарий противоречил коду.** `awg3.go:113-115` утверждал, что
   AWG3-узел сохраняет прописанный сервером MTU и потолок AWG2 к нему не
   применяется, — а `hasAWGParams` возвращал `hasAWG3Params`, то есть AWG3
   клампился наравне со всеми. Из того же заблуждения выросла и правка
   `node_parser_amnezia.go:372`: она специально поднимала `mtu: 1376` из
   `last_config` «чтобы не было клампа», а кламп тут же возвращал 1280 —
   работа отменяла сама себя. Правка ОСТАВЛЕНА: её настоящая польза к AWG
   отношения не имеет — у ОБЫЧНОГО WG-профиля потолка нет, и без неё его
   узел терял прописанный сервером MTU целиком. Переписано обоснование.

**Форма правила — два новых атрибута.** Потолок здесь диктует не поле, а
РОД узла, и обычным `max` это не выражается: он снял бы `mtu` у каждого
обычного WireGuard-узла. Поэтому `max_when` (условный потолок) и условие
`when.any_set` у него и у `default_when`.

**Условие смотрит на НАЛИЧИЕ ключа, а не на непустоту значения** — в
отличие от `conflicts`/`requires`, где ядро судит по значению. Разница не
косметическая: `jc: 0` — законная запись «мусорные пакеты выключены» у
настоящего AmneziaWG-узла (кейс корпуса `awg_jc_zero_explicit`), и прочитай
условие её как «поля нет», с такого узла потолок снялся бы и вернулась бы
ровно та тихая поломка, от которой правило заведено.

**Парность входов нарушена НАМЕРЕННО — решение владельца 18.09.2026.**
Вход `singbox` внесён в `except_sources`: тело в форме ядра значение
СОХРАНЯЕТ и получает `awg_mtu_high` (`info`), все прочие входы (ссылка
wg/awg, `.conf`, `awg://base64`, форма AWG) получают замену на 1280 и
`awg_mtu_clamped` (`warning`). Основание не техническое, а по владению:
тело sing-box человек или подписка написали САМИ в собственной форме ядра,
и молча переписывать это лаунчер не вправе — он предупреждает; значение из
ссылки сочинял генератор провайдера. Подстановка дефолта 1280 при
ОТСУТСТВИИ `mtu` кода не даёт и действует на всех входах, включая
`singbox`: это дефолт, а не замена написанного.

Это ЕДИНСТВЕННОЕ место контракта, где вход узла влияет на результат, и
потому оно требует отдельной осторожности: вход обязан определяться
СТАБИЛЬНО. Он живёт в двух местах — на разборе `ParsedNode.Source`, в
состоянии `Origin.Kind`, — и перевод между словарями один на оба пути
(`subscription.NodeSourceFromOriginKind`). Пересчёт кодов при загрузке
state читает `Origin.Kind` и вправе переписать тело; не знай он входа,
JSON-узел с `mtu: 1420` был бы заклампен задним числом — то есть настройка
пользователя исчезала бы от перезапуска, молча и необратимо. Закрыто
тестом `TestPipelineAWGMTUExceptionSurvivesStateReload`.

Xray-вход к AmneziaWG неприменим: Xray-массив wireguard не поддерживает и
даёт ноль узлов (`TestXrayUnsupportedProtocolStaysSeparate`).

**Побочно закрыт дрейф схемы.** `schema/registry_body.schema.json` при
`additionalProperties: false` не знала `default_when`, `normalize_code`,
`advisory.except`/`advisory.when`, `relation.equals` и формат `base64_32` —
то есть отвергала ЖИВОЙ реестр, хотя Go-линтер эти атрибуты проверял.
Схема догнана, весь реестр по ней теперь валиден.

Корпус: тела узлов НЕ изменились нигде; у восьми AWG-кейсов с исходным
`mtu > 1280` добавился код. Парные кейсы намеренно РАЗНЫЕ —
`body/singbox/endpoints_awg_mtu_high` (тело 1420 + `awg_mtu_high`) против
`uri/wireguard/awg_mtu_1420_clamped` (тело 1280 + `awg_mtu_clamped`);
`body/singbox/endpoints_awg_mtu_default` закрывает дефолт на JSON-входе.
Встречная задача LxBox — `TASKS_LXBOX.md` §24.14.
...
## 10. Решение владельца 18.09.2026: грамматика ядра не дублируется (контракт 1.1.9)

Развилка, из которой родились SPEC 132 и правило формы `vless.encryption`.

**Предыстория.** Сессия ядра сняла по исходникам полную грамматику
постквантового слоя VLESS — восемь шагов разбора, коэффициенты padding-блоков,
ключи 32 и 1184 байта, `ByteDecode₁₂` для ML-KEM с проверкой каждого из 768
коэффициентов. Всё это легло справкой в
`SPECS/131-F-O-UNIFIED_NODE_PIPELINE/VLESS_ENCRYPTION_GRAMMAR.md`. Напрашивалось
внести её в реестр правилом.

**Решение: НЕ вносить.** Копия грамматики ядра в клиенте обязана сверяться на
каждом бампе пина, а отставшая копия начинает отбраковывать **годные** узлы —
то есть вредит больше, чем ловит. Это тот же класс ошибки, что уже случался с
`utls_fp_allowlist` и с `reality_key_share`, только цена выше: там снималось
поле, здесь хоронился бы узел.

**Что вместо неё — два эшелона.**

1. **Правило ФОРМЫ в реестре** (дёшево, не расходится с ядром): нормативный
   порядок «декодировать → обрезать края → `none`? → `pattern`», выражение
   `^mlkem768x25519plus(\.[^.]+){3,}$`, отказ = `drop_node`
   `vless_encryption_invalid`. Судится только имя метода и число непустых
   частей. Вид, rtt, padding и ключи — не судятся вовсе; пробелы ВНУТРИ
   сегментов законны (ядро тримит сегменты с четвёртого).
2. **Страховка «ядро отвергло узел»** (SPEC 132, общая норма с LxBox 478):
   ядро назвало узел → приложение выключает его обычным `enabled=false` +
   причина → пересборка. Норма — `contract/docs/PARSING_PRINCIPLES.md` §9, код
   `core_rejected`, поле `disabled_reason`.

Эшелон 1 ловит грубое и мгновенно, эшелон 2 — всё остальное и без единой
строки грамматики в клиенте.

### 10.1 Побочное: `none` теперь сравнивается ТОЧНО

Прежде маппер снимал `none` через `EqualFold`+trim, то есть `None`/`NONE`
считались «слоя нет». Это было **скрытой дырой**: ядро сличает свой литерал с
учётом регистра и до trim, поэтому `None` для него — настоящее значение, на
котором падает ВЕСЬ конфиг. Клиент, тихо превращавший `None` в «слоя нет»,
не спасал узел, а **прятал** негодное значение от собственных проверок и
отдавал его ядру на другом входе (тело sing-box маппера не имеет вовсе).

Теперь сравнение точное на всех трёх входах, и `None` одинаково уходит в
`drop_node`. Запись в секции `mapper` про «`none` любого регистра» снята:
переводить нечего. Ни один существующий кейс корпуса от прежнего поведения не
зависел — проверено до правки.

### 10.2 Два новых атрибута реестра

- **`pattern`** — регулярка на строковое значение, после `normalize`. Диалект
  = общее подмножество Go RE2 и ECMAScript/Dart; якоря `^…$` живут в самом
  выражении. Санитайзер невалидное выражение **пропускает** (реестр вправе
  уехать вперёд кода), поэтому компиляцию, якоря и запрещённые конструкции
  обязан ловить линтер — иначе опечатка в контракте молча отключила бы
  проверку.
- **`absent_values`** — литералы «этого нет»: после `normalize`, до
  ограничений, поле не пишется, кода нет; сравнение точное и только
  строковое. Без него правило судило бы выключатель `none` как строку
  грамматики и хоронило узел за выключенную настройку — ровно то, что и
  вылезло на теле sing-box при первом прогоне корпуса.

### 10.3 Протухшая запись, попутно исправлена

У URI-параметра `vless.encryption` стояли `ext: "mobile"` и заметка «Go
параметр НЕ парсит». Обе неверны с момента появления записи: Go читает
параметр в `core/config/subscription/node_parser_core.go:770-775`. В
документации это выглядело как «Supported by LxBox only». Снято.

---

## 11. Решение владельца 19.09.2026: ключи WireGuard и правила AWG судит ТОЛЬКО реестр (контракт 1.1.11)

Дословно: «Парсер ссылки сам проверяет ключи WireGuard… я считаю так нельзя.
Должно быть снятие реестром с сообщением, а не тихое. Никаких копий в старых
эмиттерах не должно быть — все проверки должны идти по новой схеме».

Закрыты находки `LEGACY_AUDIT` №4, №8, №9, №27 и четыре запроса агента LxBox
по AmneziaWG.

### 11.1 Ключи WireGuard (находка №4)

`normalizeWGKey` был судьёй: не-32-байтный ключ он возвращал ошибкой, а
вызывающий ронял узел **молча**. Три беды те же, что у MTU: узел исчезал без
причины; тело sing-box проверки не проходило вовсе и уносило тот же мусор в
ядро; правило нельзя было ни прочесть, ни сверить со второй стороной.

Теперь `normalizeWGKey` — **перевод написания и ничего больше** (четыре
варианта base64 → std с паддингом; записан в секцию `mapper` реестра как
`wg_key_spelling_to_std_base64`). Годность судят
`private_key` / `peers[].public_key` / `peers[].pre_shared_key`:
`format: base64_32`, `on_invalid: drop_node`, новый код **`wg_key_invalid`**
(severity `error`, params `path` + `value`; у секретных полей значение
маскируется до `***`). Отсутствие поля по-прежнему даёт `field_missing` —
это разные вопросы, и коды у них разные.

**`pre_shared_key` роняет УЗЕЛ, а не поле.** Реестр до этого снимал у него
поле (`drop`/`type_invalid`), а парсер ссылки ронял узел — расхождение внутри
одного правила. Решение владельца: узел. Туннель без ожидаемого сервером PSK
— тихо сломанный туннель: рукопожатие не пройдёт, а узел выглядит рабочим и
молчит. Корпус LxBox отбраковки уже ждал.

### 11.2 AWG: четыре запроса LxBox

1. **`awg_range` без границ.** Тип объявлял «число или min-max» и не говорил,
   каких размеров бывают границы; своп перевёрнутой пары делал код Go.
   Теперь границы — **uint32** (шире ядро отвергает разбором, роняя весь
   конфиг), а своп — режим нормализации **`normalize: "range_order"`**,
   **тихий**: порядок границ смысла не несёт (ядро выбирает значение ИЗ
   диапазона, `[10,40] = [40,10]`), а замена без смены смысла кода не даёт —
   то же правило, что у `trim`. Флаг стоит только у `h1`–`h4`; у таймингов
   AWG 3.x его нет намеренно — там перевёрнутая пара опечатка человека, и он
   обязан её увидеть (SPEC 123 §2).
2. **Битые `jc`/`jmin`/`jmax`/`s1`–`s4` (находка №9).** Go ронял поле МОЛЧА, а
   реестр объявлял `on_invalid.code` — то есть контракт обещал код, которого
   не существует. Асимметрия жила внутри ОДНОЙ функции: `h1`–`h4` код
   получали, соседи по циклу — нет. Теперь маппер переносит значение как
   есть, а поле снимает санитайзер с кодом реестра. Тела узлов не сдвинулись:
   в корпусе изменились только `warnings`.
3. **`presharedkey`** — см. §11.1.
4. **AWG 3.x (находка №8).** Коды `awg3_header_key_invalid`,
   `awg3_padding_too_short`, `awg_headers_overlap` были объявлены в
   `warnings.json` и **не ставились никогда**: рукописные `validateAWG3` и
   `awgHeaderOverlap` роняли узел молча и только на входе «ссылка». Переведены
   в реестр целиком:
   - негодный `header_protection_key` → `on_invalid: drop_node`. Запрет «все
     нули» выражен `pattern`'ом: строка из одних `A` с паддингом и есть 32
     нулевых байта, а `format: base64_32` про содержимое ничего не знает;
   - порог `s1..s4 >= 12` при заданном ключе → новый атрибут **`min_when`**
     (зеркало `max_when` без исключения по входу и без замены значения) с
     флагом **`absent_is_zero`**: ядро читает незаданный `s2` как 0, и «ключ
     есть, паддинга нет» так же фатально, как «ключ + паддинг 5». Без флага
     правило молчало бы ровно на том случае, который в живых подписках
     встречается чаще битого значения;
   - пересечение `h1..h4` → новая секция **`body.relations`** с видом
     **`ranges_disjoint`**. Связь между НЕСКОЛЬКИМИ полями `conflicts`/
     `requires` не выражают: те говорят про пару «я и сосед», а здесь
     свойство набора — виноват может быть любой из четырёх, и снятие одного
     пару не развело бы. Незаданный заголовок участвует своим дефолтом ядра
     (`h1=1 … h4=4`) — атрибут `defaults`.
5. **Дефолт `allowed_ips` (находка №27).** `0.0.0.0/0,::/0` стоял ДВУМЯ
   копиями (парсер ссылки и конвертер профиля Amnezia) и на входе sing-box не
   работал вовсе: тело без `allowed_ips` узел терял, хотя ядро отвергает
   такого пира фаталом на весь конфиг. Теперь это `default_when` реестра, обе
   копии сняты. Тела узлов не изменились.

### 11.3 Побочное: связь читает ЧИСТУЮ карту, а не исходную

Первая версия `ranges_disjoint` заглядывала в исходное тело, когда поля нет в
чистом. Это неверно: поле, снятое санитайзером за негодное значение, в тело
не поедет, и ядро прочтёт вместо него свой дефолт. Живой симптом поймал кейс
`awg_h1_above_uint32_dropped`: снятый `h1 = "1-4294967296"` продолжал
«пересекаться» с соседями и хоронил узел кодом `awg_headers_overlap` вместо
честного `awg_header_invalid` на самом поле — **вина уезжала не на того**.

### 11.4 Побочное: у записи отбраковки появился `code`

`code` в `dropped[]` нормативен (D-088), `reason` — нет. Раннеры корпуса при
этом знали только текст ошибки Go, и любая отбраковка приезжала в конверт
...
есть проверить перенос правил в реестр было нечем. `canonNodeDrop` берёт код
у отказа санитайзера; попутно коды получили отбраковки и других схем
(`vless_encryption_invalid`, `field_missing` у naive).

---

## 12. Старый per-scheme эмиттер снят (контракт 1.1.11)

Вторая половина решения владельца 19.09.2026: «Никаких копий в старых
эмиттерах не должно быть — все проверки должны идти по новой схеме».

### 12.1 Что снято

- **`GenerateNodeJSONBare`, ветка per-scheme** — 366 строк цепочки `else if`,
  вручную воспроизводившей порядок полей структур ядра. Тело теперь берётся у
  `materializeParsedNodeBody`, а `tag`/`detour` возвращает на места общий
  `stampTagAndDetour` (он же обслуживает узел с готовым телом).
- **`outbound_tls_emit.go` целиком** (172 строки). Он держал копии
  `utls_fp_unknown`, `reality_key_share_invalid` и правил `flow` — вторые
  экземпляры того, что уже описано реестром. Из файла пережила одна функция
  `mapStringValue`, переехавшая в `node_materialize.go`.
- **`obfsIntField`, `tolerantInt`, `tolerantStringSlice`** — они существовали,
  чтобы эмиттер читал число и список из ЛЮБОЙ формы, в которой те приехали
  (`int` от URI-парсера, `float64` от разбора JSON, строка от маппера W2d);
  ловушка Л8 «JSON-карта и `.(int)`-ассерты». Приведением занимается
  `nodeflow/coerce.go` по реестру — один раз и на все схемы сразу.

`outbound_generator.go` похудел с 2115 строк до 1712.

### 12.2 🔴 Подпись миграции у ЗДОРОВЫХ узлов расходится — НЕ «редкое × редкое»

`LegacyNodeIdentityHash` переведён на новый конвейер. Владелец принимал цену
как «у узлов с МУСОРОМ в полях хэш может отличаться → редкое × редкое».
**Замер по всему корпусу URI (305 узлов) показал, что это не так**: совпали
231, разошлись 74, и среди разошедшихся мусор — меньшая часть.

Классы расхождения у **здоровых** узлов:

| Что | Было → стало | Кейсов |
|---|---|---|
| `tls.reality.short_id` | `ABCD` → `abcd` | 8 |
| `packet_encoding` | `PacketAddr` → `packetaddr` | 2 |
| `up_mbps` / `down_mbps` у hysteria | нет → `100` | 1 |
| `tls.utls.fingerprint` под REALITY | нет → `chrome` | 3 |
| `tls.reality.key_share` | нет → `classical` | 1 |

Заглавный hex в `sid` законен, `packetEncoding=PacketAddr` пишут панели,
hysteria без полосы — обычная ссылка. Приведение при этом **правильное**:
именно это тело и уезжает в ядро, и подгонять конвейер под старый эмиттер
нельзя. Но и молчаливая потеря сопоставления при миграции состояния СТАРОЙ
схемы (до SPEC 112) — не «редкий случай»: отметка выключения или
`detour_node_hash` на таком узле приедет как «узел не найден».

**Нужно решение владельца.** Варианты, которые видны:

- **А.** Оставить как есть. Цена: при ОДНОМ апгрейде со старой схемы часть
  отметок выключения слетает, узлы включаются. Чинится руками, разово,
  и только у тех, кто мигрирует с до-SPEC-112 состояния.
- **Б.** Считать legacy-подпись по «сырому» телу — до `normalize` и до
  `default_when`. Сопоставление сохраняется точнее, но требует второго режима
  санитайзера, который существует ТОЛЬКО ради миграции, — то есть ровно той
  второй копии правил, от которой волна уходит.

До решения действует А (так собрано сейчас). Байтовые голдены
`TestLegacyNodeIdentityHashByteStablePins` при этом **не сдвинулись** — все
три пина совпали, как и `LegacyHash` у обоих тестов миграции.

Отдельно: на дедуп записей подписки (`dedupSignature`, единственный другой
потребитель хука) смена эмиттера не влияет по построению — подпись живёт один
разбор одного тела, обе копии эмитятся одним кодом из одной формы, о чём в
самой функции и написано.

### 12.3 ЗАКРЫТО (контракт 1.1.12): `tls:{enabled:false}` — правило реестра

SPEC 045: явный disabled-блок ронял ядра 1.14.0-lx.5..lx.18 в SIGSEGV. DRIFT
§2 числил правило ТРОЙНОЙ копией (парсер ссылки, `singbox_sanitize`,
`outbound_tls_emit`). Третья снята вместе с эмиттером; две живые остались, то
есть **на обоих живых входах правило работает**.

В реестре его нет, и существующей грамматикой оно не пишется: нужен атрибут
«такое значение поля означает, что ВЕСЬ объект не задан», а `absent_values`
про другое (он про само поле) и строковый. Тело, пришедшее с
`tls:{enabled:false}` мимо обоих мапперов (ручной JSON вкладки, чужой бэкап),
доедет до конфига как есть. Заводить атрибут посреди этой волны, без ревью
второй стороны, значило бы делать ровно то, от чего кампания уходит.

**Закрыто отдельным заходом (контракт 1.1.12).** Атрибут заведён как
`absent_when: {"enabled": false}` — у поля-объекта, а у суб-схемы `tls` один
раз у секции, откуда он переезжает в каждый протокол при разрешении `ref`.
Тем же атрибутом, а не особым случаем, описаны вложенные `utls`/`reality`/`ech`.
Норма порядка — `PARSING_PRINCIPLES.md` §6.1: судится ДО правил полей объекта и ДО связей
соседей, снятый объект «не задан» для любой проверки наличия вместе со всем,
что внутри. Обе живые рукописные копии сняты (`singbox_sanitize.go` и гард
`reality["enabled"]` в `EnforceRealityFingerprint`). Тела корпуса не сдвинулись;
новый кейс — `body/singbox/tls_disabled_block`.

### 12.4 Побочное: фикстуры тестов несли креденшл мимо карты

Шесть тестов старого эмиттера заполняли `ParsedNode.UUID` и оставляли
`Outbound` без `uuid`/`password`. Старый switch читал поле структуры, конвейер
читает карту — и такой узел честно отбраковывается как «vless без uuid».
Настоящие парсеры кладут креденшл В КАРТУ (проверено на vless и trojan), так
что фикстуры описывали узел, которого не бывает; приведены к реальной форме.

Ещё один тест ждал ОМИТА негодного `fingerprint` — так делала копия правила в
снятом эмиттере. Реестр ЗАМЕЩАЕТ его каноническим `chrome` с кодом
`utls_fp_unknown`, и это осмысленнее: ядро без ключа берёт СВОЙ hello, а
подписка отпечаток указала намеренно, чтобы обойти DPI.
