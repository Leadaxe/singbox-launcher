...
## Живая проверка (этап 4.2) — 2026-08-22

Проверено на рабочей системе через debug API лаунчера (`/debug/snapshot`,
`/proxies`, `/action/ping-all`), ядро `1.14.0-lx.27-rc.3-dev`, VPN не
прерывался.

**Конфиг собран верно.** Все три XHTTP-узла подписки несут поля из `extra`:

| Узел | mode | поля транспорта сверх базовых |
|---|---|---|
| 🇩🇪 Германия БС-5 | `packet-up` | `x_padding_bytes`, `uplink_chunk_size`, `uplink_http_method`, `sc_max_each_post_bytes`, `sc_stream_up_server_secs`, `sc_max_buffered_posts` |
| 🇨🇭 Швейцария БС-3 | `stream-one` | `x_padding_bytes`, `xmux` |
| 🇨🇭 Швейцария БС-4 | `stream-one` | `x_padding_bytes`, `xmux` |

Числа из `extra` приехали строками (`"1000000"`, `"0"`), без `1e+06` — то,
ради чего этап 3.3 и делался. `sc_max_buffered_posts: 30` осталось числом,
как в источнике.

**Задержки** (`/action/ping-all`, два прогона):

| Узел | прогон 1 | прогон 2 |
|---|---|---|
| 🇨🇭 Швейцария БС-3 | 375 мс | 361 мс |
| 🇨🇭 Швейцария БС-4 | 387 мс | 402 мс |
| 🇩🇪 Германия БС-5 | −1 | −1 |

**Немецкий узел не проходит URL-тест, и это не дефект разбора.** TCP-порт
443 на `188.72.103.4` открыт, конфиг узла корректен, но рядом мертва и
`🇩🇪 Германия БС-1` — обычный vless без XHTTP. Всего в конфиге 6 мёртвых
vless-узлов из 33, XHTTP среди них ровно один: если бы ветка XHTTP была
сломана, легли бы все три, а не один из трёх. Причина на стороне сервера
(«БС» = белые списки, узлы флапают — см. `liberty-flap-diagnosis`).

Вывод: фикс подтверждён на живой системе. Оба швейцарских узла работают
через XHTTP с полями `extra`, немецкий отказывает по причине, не связанной
с разбором.
