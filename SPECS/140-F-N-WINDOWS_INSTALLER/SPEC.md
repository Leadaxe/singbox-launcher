...
- **139 — обязательная предпосылка.** С `requireAdministrator` «Launch after install» от
  ...
  неповышенного родителя (`SINGBOX_LAUNCHER_DATA_DIR`/`…_LOG_DIR`), а не вычисляет заново —
  иначе проба записи Program Files вернёт Legacy на остатке из ловушки (а), а повышение под
  чужой учётной записью — её `%LOCALAPPDATA%`.
