...
- **Окно совместимости — ОТМЕНЕНО решением владельца (D-110, 14.09.2026).**
  Релизы лаунчера 1.6.0 и LxBox 2.24.0 синхронны: обе стороны пишут только
...
- `contract/VERSION` → `1.0.0` — **после** того как LxBox читает и пишет 1.0
  ...
  чтение 1.0. Это принятое следствие порядка §4. **Выполнено**: LxBox 2.24.0
...
бэкап десктоп ↔ телефон на синхронных релизах (лаунчер 1.6.0, LxBox 2.24.0 —
...
| — | приёмка (SPEC 126 §4) | **ждёт синхронных релизов**: лаунчер 1.6.0, LxBox 2.24.0 | |

**Что ещё не закрыто и почему:**

- ~~`contract/VERSION` = `0.12.11`~~ — поднят до `1.0.0` (D-110): LxBox 2.24.0
