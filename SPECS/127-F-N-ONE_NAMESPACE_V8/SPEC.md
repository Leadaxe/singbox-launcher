...
(утверждена владельцем 14.09.2026, D-104/D-105). Карта кода — `CODEMAP.md`
...
- Секции узла едут в бэкап **только в 1.0** (норма §4): экспорт 0.x
  лаунчер больше не пишет.
...
- `TASKS_LXBOX.md` `## 14` — контракт 1.0: что изменилось в схеме, что
  ждём (чтение 1.0 + legacy 0.x, запись 1.0, корпус зелёный).
- Решение D-106 — норма контракта 1.0 (после реализации).

## 6. Открытый вопрос владельцу (один)

Канон разбора узла (`contract/schema/node.schema.json`, `corpus/uri/**`,
`corpus/body/**`) называет тело узла `entry`. По принципу единого имени
это `body`. Переименование трогает все файлы корпуса и раннеры обеих
сторон. Вопрос: переименовать `entry` → `body` в этой же кампании или
оставить `entry` в каноне разбора как термин «запись разбора» (не
хранения)?
...
3. Волна 2 — бэкап 1.0 (экспорт = состояние, legacy-вход 0.x) + round-trip
   + корпус legacy.
4. Волна 3 — контракт: схема, документы, корпус 1.0, TASKS_LXBOX `## 14`,
   D-106; пинг LxBox с хэшем.
