...
бэкап десктоп ↔ телефон, а он требует чтения 1.0 у LxBox, которого ещё нет.
Код и контракт со стороны лаунчера закрыты.
...
| 3 | контракт 1.0: две схемы, документы, корпус 1.0, `TASKS_LXBOX ## 16`, D-109; этап FIX по ревью | сделано | ветка `spec127/contract-10` от `9fc880e1` |
| — | приёмка (SPEC 126 §4) | **ждёт LxBox**: чтение 1.0 — их волна 3 | |

**Что ещё не закрыто и почему:**

- `contract/VERSION` = `0.12.11`. Поднимается до `1.0.0`, только когда LxBox
  читает И пишет 1.0 — правило обеих сторон (D-109). В `contract/README.md`
  строка `1.0.0` стоит с пометкой «черновик; VERSION НЕ поднят».
- Дефолт экспорта — `ExportFormat012` (одна константа
  `backup.BackupExportFormatDefault`). Переключается на `ExportFormat10`
  после релиза LxBox с чтением 1.0; версию и хэш такого релиза мы запросили
  в `TASKS_LXBOX ## 16` §16.5.
- Окно совместимости живо: обе стороны читают оба формата всегда.
