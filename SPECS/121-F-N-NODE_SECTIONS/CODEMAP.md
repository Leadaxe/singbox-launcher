...
> Примеры и имена полей ниже показывают форму v7: `order_num` → `num`, `name`
> и `refs[]` — поля записи, `body` — объект sing-box как есть, у DNS-сервера
> тег в поле `tag`. Действующие адреса пакета `core/state` —
> `SPECS/127-F-N-ONE_NAMESPACE_V8/CODEMAP.md` §8, UI — §10 там же.
