...
### 10.3 Чтение старого формата (ничего не терять)

Сборка с форматом волн 1–2 уже ставилась владельцу; `state.json` и файлы
бэкапа могут нести старую форму. При чтении (`normalizeNodeShape`, импорт
бэкапа) старая форма распознаётся по ключам `dns_servers`/`dns_rules` или
по `rules` как массиву **sing-box-объектов без `kind`** и переводится один
раз, без потерь эмитируемого конфига:

- `dns_servers[i]` → `dns.servers[i]` вида `user`; локальный тег `X` →
  `@{self}:X` (ровно то, что давала неявная префиксация `<финальный>:X`);
- `dns_rules[i]` → `dns.rules[i]` вида `user`; `server: X`, равный
  локальному тегу сервера, → `@{self}:X`;
- `rules[i]` (sing-box-правило) → запись `inline`: `match` = все поля кроме
  `outbound`/`action`, `outbound` = было (`@self`, если отсутствовал),
  `name` = `@{self} rule N`; `enabled`/`order_num` — из записи `kind=node`
  этого узла в `rules[]` состояния, если она есть (после чего запись
  `kind=node` удаляется), иначе `true`/945.
- Бэкап: `servers[].sections` старой формы (+`rule_num`) переводится тем же
  конвертером; `rule_num` → `order_num` первой записи.

Конвертер — одна функция в `core/state` с тестом на обеих формах.
...
- **Вкладка JSON узла.** Показывает `{ "endpoints"|"outbounds": [тело],
  ...
  хранимую форму конвертером §10.3 (правило → `inline`, сервер/правило →
  `user`). `ParseNodeDocument` возвращает `*state.NodeSections` новой формы.
...
`servers[].sections` — объект §10.1 без переименований; `rule_num`
упраздняется. `contract/schema/backup.schema.json`, `contract/docs/BACKUP.md`
...
`state_directory` только в конфиг; ребро `endpoint` в санитайзере; чтение
старого формата §10.3; тесты волн 1–2 и SPEC 122 переписаны на новую форму,
