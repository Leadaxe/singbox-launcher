...
### Выжимка (RU) — Windows без прав администратора по умолчанию (ветка `spec-139-asinvoker`, версия не назначена)

**Реализовано в ветке, ждёт приёмки на Windows** (SPEC 139, #99). Лаунчер на Windows больше не спрашивает UAC на каждом старте: он работает без прав администратора, режиму прокси они не нужны. **TUN** их требует — Start без прав предлагает **Перезапустить от имени администратора** (один запрос UAC, те же данные, VPN поднимается сразу) или **Перейти в режим прокси**. Появился **автозапуск**: Settings → Connection → «Запускать вместе с Windows» и «Подключать VPN при входе». **Распаковывайте zip в папку, куда вы можете писать** (не в `Program Files`): там `portable.txt` теперь не учитывается, и данные живут в `%LOCALAPPDATA%\singbox-launcher` (данные старой версии копируются туда сами). Подробности: [README.ru.md — Установка → Windows](README.ru.md#windows-1), [docs/TROUBLESHOOTING.ru.md](docs/TROUBLESHOOTING.ru.md#windows).

### Highlights (EN) — Windows runs without administrator rights by default (branch `spec-139-asinvoker`, version not assigned yet)

**Implemented on the branch, pending acceptance on Windows** (SPEC 139, #99). The launcher on Windows no longer asks for UAC on every start: it runs without administrator rights, and proxy mode needs none. **TUN** does — Start without rights offers **Restart as administrator** (one UAC prompt, the same data, the VPN starts right away) or **Switch to proxy mode**. **Start with Windows** is new: Settings → Connection → “Start with Windows” and “Connect VPN at sign-in”. **Extract the zip to a folder you can write to** (not `Program Files`): there `portable.txt` is now ignored and data lives in `%LOCALAPPDATA%\singbox-launcher` (an older version's data is copied there automatically). Details: [README.md — Installation → Windows](README.md#windows-1), [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md#windows).

### Выжимка (RU) — установщик для Windows (ветка `spec-140-installer`, версия не назначена)

**Реализовано в ветке, ждёт SPEC 139 и приёмки на Windows** (SPEC 140). Новый ассет `singbox-launcher-<версия>-win64-setup.exe` для Windows 10/11 x64: один запрос прав администратора, программа — в `C:\Program Files\singbox-launcher`, данные — в `%LOCALAPPDATA%\singbox-launcher`; ядро, `wintun.dll`, шаблон и Mesa3D внутри, первый запуск ничего не качает. Ярлык в «Пуске» и строка в «Параметры → Приложения»; на странице задач — ярлык на рабочем столе, **«Запускать вместе с Windows»** (только при первой установке, дальше это чекбокс в настройках лаунчера) и программный OpenGL (Mesa3D) для RDP / ВМ без видеокарты. Установка новой версии поверх сохраняет данные и сама закрывает работающий лаунчер — VPN останавливается штатно, системный прокси снимается. **Удаление спрашивает, удалить ли и данные** (по умолчанию — нет): удаляются настройки, подписки и логи учётной записи, от которой запущено удаление. Установщик пока не подписан — SmartScreen может попросить «Подробнее → Выполнить в любом случае». Zip-архивы остаются portable: распаковывайте их в папку с правом записи, а не в Program Files.

### Highlights (EN) — Windows installer (branch `spec-140-installer`, version not assigned yet)

**Implemented on the branch, pending SPEC 139 and acceptance on Windows** (SPEC 140). A new asset `singbox-launcher-<version>-win64-setup.exe` for Windows 10/11 x64: one administrator prompt, the program goes to `C:\Program Files\singbox-launcher`, your data to `%LOCALAPPDATA%\singbox-launcher`; the core, `wintun.dll`, the template and Mesa3D are inside, so the first start downloads nothing. A Start menu shortcut and an entry in Settings → Apps; the tasks page offers a desktop shortcut, **Start with Windows** (first install only — afterwards it is the checkbox in the launcher settings) and software OpenGL (Mesa3D) for RDP / VMs without a GPU. Installing a newer version over it keeps the data and closes a running launcher by itself — the VPN stops cleanly and the system proxy is cleared. **Uninstall asks whether to remove the data too** (no by default): the settings, subscriptions and logs of the account that runs the uninstaller. The installer is not code-signed yet — SmartScreen may ask for *More info → Run anyway*. The zip archives stay portable: unpack them into a folder you can write to, not Program Files.
