...
### Выжимка (RU) — следующий релиз (версия не назначена)

**Шаблон конфигурации (SPEC 143).** Если в настройках шаблона что-то не так — значение не число там, где ждётся число, число вне диапазона, ссылка на незаявленную переменную или неизвестная директива, правило или DNS-сервер пресета выпали из-за пустой настройки, — это теперь видно первыми строками «Итога» в отчёте сборки; сохранение это не блокирует. На вкладке **Настройки** переменные со списком допустимых значений показываются выпадающим списком (или комбобоксом, если шаблон разрешает свой ввод), а числовые поля (MTU, порты) сразу подсвечивают не-число. Свои шаблоны, где MTU или порт объявлены `type: text`, теперь подставляют строку — объявите `type: int`.

### Highlights (EN) — next release (version not assigned yet)

**Config template (SPEC 143).** When something is wrong in the template settings — a non-number where a number is expected, a number out of range, a reference to an undeclared variable or an unknown directive, a preset rule or DNS server dropped because of an empty setting — it now shows as the first lines of the Summary in the build report; saving is not blocked. On the **Settings** tab, variables with a list of allowed values show as a dropdown (or a combo box when the template allows free input), and numeric fields (MTU, ports) flag a non-number right away. Custom templates that declare MTU or a port as `type: text` now substitute a string — declare `type: int`.
