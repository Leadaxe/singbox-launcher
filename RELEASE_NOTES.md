...
**Implemented on the branch, pending core lx.12 and owner acceptance** (SPEC 136) — macOS daemon mode only. The daemon service used to run as root from a file your user account can modify (the core in the bundle or in the data folder). Since core lx.11 the install command copies the core into the file `/Library/PrivilegedHelperTools/sing-box-lxd` (owned by root) and points the service at the copy. The launcher checks the service without sudo: an unsafe one gets a red notice on the LOCAL tab and one dialog per launcher version, an outdated one a yellow notice; the VPN is not blocked. One command covers every case — **Install or update service**: first install, an old service, the update after downloading a new core. Details: [docs/DAEMON_AND_REMOTE.md §2.1](docs/DAEMON_AND_REMOTE.md).
