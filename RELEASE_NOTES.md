...
**Реализовано в ветке, ядро запинено на lx.12, ждёт приёмки владельцем** (SPEC 137) — только macOS, classic-режим с TUN. Ядро под root запускалось из файлов, которые может изменить ваша учётная запись (скрипт в папке данных и ядро лаунчера), и после первого пароля — без вопросов на каждом рестарте. Теперь root запускает только копию ядра `/Library/PrivilegedHelperTools/sing-box-lxd` (та же, что у службы демона) и системные утилиты. При первом старте с TUN после обновления лаунчер попросит выполнить одну команду (`sudo … lxd --service=copy`) и нажать «Повторить»; после обновления ядра — снова. Root больше не пишет в папки пользователя: лог ядра с TUN — `/Library/Logs/sing-box-lxd/classic.log`, снятие TUN — без пароля. Закрыто на macOS; Windows — следующим. Подробности: [docs/DAEMON_AND_REMOTE.ru.md §2.2](docs/DAEMON_AND_REMOTE.ru.md).

### Highlights (EN) — classic TUN runs a root-owned copy of the core (branch `spec-137-classic-root-owned`, version not assigned yet)

**Implemented on the branch, core pinned to lx.12, pending owner acceptance** (SPEC 137) — macOS classic mode with TUN only. The core used to run as root from files your user account can modify (a script in the data folder and the launcher's core), and after the first password — without asking on every restart. Now root runs only the copy of the core, `/Library/PrivilegedHelperTools/sing-box-lxd` (the same one the daemon service uses) and system utilities. On the first TUN start after the update the launcher asks you to run one command (`sudo … lxd --service=copy`) and click Retry; after a core update it asks again. Root no longer writes into user folders: the core log with TUN is `/Library/Logs/sing-box-lxd/classic.log`, and turning TUN off asks no password. Closed on macOS; Windows comes next. Details: [docs/DAEMON_AND_REMOTE.md §2.2](docs/DAEMON_AND_REMOTE.md).

### Выжимка (RU) — служба демона на root-owned копии ядра (ветка `spec-136-daemon-root-owned`, версия не назначена)

**Реализовано в ветке, ядро запинено на lx.12, ждёт приёмки владельцем** (SPEC 136) — только macOS, daemon-режим. Служба демона запускалась от root из файла, который может изменить ваша учётная запись (ядро в бандле или в папке данных). С ядра lx.12 команда установки копирует ядро в файл `/Library/PrivilegedHelperTools/sing-box-lxd` (владелец root) и переводит службу на копию. Лаунчер проверяет службу без sudo: небезопасная — красная плашка на вкладке LOCAL и один диалог на версию лаунчера, служба не на ядре лаунчера — жёлтая; VPN при этом не блокируется. Для всех случаев одна команда **Install or update service** — первая установка, старая служба, обновление после скачивания ядра; её даёт только ядро лаунчера lx.12+, со старым лаунчер сначала просит обновить ядро (вкладка «Локально» → «Скачать/Переустановить»). Подробности: [docs/DAEMON_AND_REMOTE.ru.md §2.1](docs/DAEMON_AND_REMOTE.ru.md).

### Highlights (EN) — the daemon service runs a root-owned copy of the core (branch `spec-136-daemon-root-owned`, version not assigned yet)

**Implemented on the branch, core pinned to lx.12, pending owner acceptance** (SPEC 136) — macOS daemon mode only. The daemon service used to run as root from a file your user account can modify (the core in the bundle or in the data folder). Since core lx.12 the install command copies the core into the file `/Library/PrivilegedHelperTools/sing-box-lxd` (owned by root) and points the service at the copy. The launcher checks the service without sudo: an unsafe one gets a red notice on the LOCAL tab and one dialog per launcher version, one that runs a different core than the launcher a yellow notice; the VPN is not blocked. One command covers every case — **Install or update service**: first install, an old service, the update after downloading a new core; it comes only from a launcher core lx.12+, with an older one the launcher asks to update the core first (Local tab → Download/Reinstall). Details: [docs/DAEMON_AND_REMOTE.md §2.1](docs/DAEMON_AND_REMOTE.md).

### Выжимка (RU) — новая раскладка данных (ветка `spec-135-impl`, версия не назначена)

**Реализовано в ветке, ждёт CI и приёмки владельцем** (SPEC 135) — данные больше не живут только рядом с исполняемым файлом. Теперь три роли: программа (только чтение), данные и логи — на Linux это `~/.local/share/singbox-launcher` и `~/.local/state/singbox-launcher/logs` (XDG, решает issue #85 про NixOS/Guix/Flatpak/snap), на macOS вне бандла — `~/Library/Application Support` и `~/Library/Logs`, на Windows — `%LOCALAPPDATA%`. Новый раздел **Settings → Storage**: где что лежит, кнопки Open и Copy paths, переключатель **Portable mode** (данные рядом с программой — как раньше, включён по умолчанию во всех zip для Windows), кнопка **Remove all data…** для полного и чистого удаления. То же самое без GUI: флаги `-paths` (напечатать пути и выйти) и `-purge-data [-yes]` (удалить данные и логи). Данные из старого расположения переносятся автоматически при первом запуске; **на macOS перед обновлением рекомендуется сделать LX Backup** (Settings → Backup) или обновляться через `build_darwin.sh -i` — перетаскивание нового `.app` поверх старого в Finder стирает данные бандла до того, как новый код успевает их перенести. На Linux изменился порядок поиска ядра: системный `sing-box` из `PATH` теперь проверяется **последним**, а не первым — используйте `SINGBOX_LAUNCHER_CORE`, если нужен именно он. Подробности и таблица путей: [docs/ARCHITECTURE.md §7a](docs/ARCHITECTURE.md#7a-data-directory-layout-spec-135), [README.md — Where data lives](README.md#where-data-lives).

### Highlights (EN) — new data directory layout (branch `spec-135-impl`, version not assigned yet)

**Implemented on the branch, pending CI and owner acceptance** (SPEC 135) — data no longer lives only next to the executable. Three roles now: program (read-only), data, and logs — on Linux that's `~/.local/share/singbox-launcher` and `~/.local/state/singbox-launcher/logs` (XDG, closing issue #85 about NixOS/Guix/Flatpak/snap), on macOS outside the bundle it's `~/Library/Application Support` and `~/Library/Logs`, on Windows it's `%LOCALAPPDATA%`. A new **Settings → Storage** section shows where everything lives with Open and Copy paths buttons, a **Portable mode** toggle (data next to the program, as before — on by default in every Windows zip), and a **Remove all data…** button for a clean full uninstall. The same without a GUI: `-paths` (print paths and exit) and `-purge-data [-yes]` (remove data and logs). Data from the old location is migrated automatically on first launch; **on macOS, take an LX Backup before upgrading** (Settings → Backup) or update in place with `build_darwin.sh -i` — dragging a new `.app` over the old one in Finder wipes the bundle's data before the new code ever gets a chance to move it. On Linux the core search order changed: a system `sing-box` on `PATH` is now checked **last**, not first — set `SINGBOX_LAUNCHER_CORE` if you specifically need that one. Details and the paths table: [docs/ARCHITECTURE.md §7a](docs/ARCHITECTURE.md#7a-data-directory-layout-spec-135), [README.md — Where data lives](README.md#where-data-lives).
