...
**Десктоп-платформа сетевой маршрутизации и анализа трафика. 15+ VPN-протоколов, глубина настроек и API уровня Enterprise. Поверх форка [sing-box-lx](https://github.com/Leadaxe/sing-box-lx) (upstream sing-box + XHTTP + AmneziaWG 2.0) как execution-engine; Windows 7 32-бит остаётся на upstream [SagerNet/sing-box](https://github.com/SagerNet/sing-box).**
...
- **Self-update** — pinned версия ядра (форк sing-box-lx, сборка с XHTTP + AmneziaWG; Win7/386 остаётся на upstream sing-box) автоматически перекачивается при mismatch; проверка self-update лаунчера при старте с уведомлением (без тихой установки).
...
- **Legacy:** Windows 7 (x86/x64) через отдельную сборку `singbox-launcher-<version>-win7-32.zip` с pinned legacy upstream sing-box 1.13.12 (32-bit, SagerNet/sing-box — без XHTTP/AWG, для `windows/386` форк-сборки нет) и 32-bit `wintun.dll`.
- [sing-box-lx](https://github.com/Leadaxe/sing-box-lx/releases) — форк-ядро (XHTTP + AmneziaWG), авто-загрузка через Core tab. Win7/386 откатывается на upstream [sing-box](https://github.com/SagerNet/sing-box/releases).
