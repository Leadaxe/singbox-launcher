...
func TestConnectionCardJSON_NoHTMLEscape(t *testing.T) {
	card, err := ConnectionCardJSON("http://127.0.0.1:9263", "abc", "v1.1.5", "1.14.0-lx.1-rc.16")
	if err != nil {
		t.Fatalf("card: %v", err)
	}
...
func TestManifestAndHelp(t *testing.T) {
	...
	ff := &fakeFacade{version: "1.14.0-lx.1-rc.16"}
	...
	if manifest.Core != "1.14.0-lx.1-rc.16" || manifest.Launcher != "v-test" {
