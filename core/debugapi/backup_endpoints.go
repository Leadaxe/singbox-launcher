...
// preset-патчами. Узлы и цепочки самого состояния сюда не добавляются, как и
// системные теги шаблона (`direct-out`, тег блокировки — они едут в
// ImportOptions.SystemTags/BlockTag): importKnownTags (core/backup/import.go)
// досчитывает их по ЖИВОМУ состоянию уже после слияния, и дублировать его
// список здесь значило бы завести вторую правду о том, что считается
// известной целью. На пустом состоянии этот список пуст — и это не «проверять
// нечем»: системные теги приёмника известны всё равно.
//
