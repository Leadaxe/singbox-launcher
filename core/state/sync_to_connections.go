...
func syncConnectionsFromLegacy(s *State) {
	...
	for _, p := range s.ParserConfig.ParserConfig.Proxies {
		...
		for j, uri := range conns {
			...
			//
			// Только у записи с ОДНИМ подключением — по той же причине, что и
			// ID ниже: legacy multi-connection ProxySource несёт один TagMask
			// на всю запись, и раздать его каждому Source значило бы назвать
			// все узлы одним тегом. Тег — идентичность (SPEC 112): ссылки
			// правил и позиций цепочек перестали бы различать узлы, а сборка
			// уникализировала бы тёзок позиционными суффиксами, каждый раз
			// заново. Остальным раздаётся per-connection тег ниже.
			if len(conns) == 1 {
