...
func refreshMergedNode(fresh *Node, old *Node) string {
	...
	// Вердикт ядра (SPEC 132, PARSING_PRINCIPLES §9.4) привязан к ТЕЛУ узла: то же тело —
	// вердикт держится и узел остаётся выключенным С объяснением; другое
	// тело — вердикт недействителен, запись стирается и узел включается
	// обратно, чтобы следующая сборка проверила его заново. Метод правит
	// Enabled, поэтому зовётся ПОСЛЕ его переноса.
	fresh.CarryCoreVerdict(old)
