...
type Node struct {
	...
	// Service — узел приехал СЛУЖЕБНОЙ частью чужой записи, а не отдельной
	// позицией списка (SPEC 120).
	//
	// Так рождаются релеи BYPASS-конфигов: провайдер прислал их не как
	// «страну для выбора», а как дозвонщика внутри конфига другого узла
	// (`sockopt.dialerProxy` у Xray). Своего имени в записи у них нет —
	// отсюда и признак служебности: узел появился довеском.
	//
	// Метка НЕ про то, что на узел кто-то ссылается: обычный сервер,
	// выбранный целью detour, остаётся обычным. Она про происхождение, и
	// ставит её импорт — там, где видно, что запись была одна, а узлов из
	// неё вышло несколько.
	//
	// Что даёт: в списке состава такой узел помечен шестерёнкой и не
	...
	//
	// С Reason не сливать (Л20): Reason — английский ТЕКСТ парсера у
	// неразобранной записи (kind=unsupported), он же едет в диагностику
	// fetch; Warnings — КОДЫ выжившего узла, которые UI переводит. Одно поле
	// на два смысла заставило бы либо переводить диагностику, либо
	// показывать пользователю английский текст парсера.
	//
	// Данные производные: пересчитываются из Origin.Raw любым повторным
	// разбором и не авторитетны при переносе (бэкап, обмен с LxBox).
	// Поэтому nil = «не считали» (узел из state до SPEC 131 — разовую
	// миграцию делает загрузка), а пустой список = «считали, чисто».
	// Различие живёт в памяти: omitempty стирает пустой список из JSON, и
	// после round-trip «чисто» читается как «не считали» — повторный счёт
	// на том же теле даёт тот же результат, так что цена ошибки нулевая.
	//
...
//
// Path пуст у кодов уровня узла (весь узел, а не поле).
//
// Своя структура, а не configtypes.Warning: state не импортирует config —
// направление импорта обратное, и общий тип завёл бы цикл. Конвертеры пары
// живут на стороне config (core/config/migrate_materialize.go).
...
type Source struct {
	...
	// выборе НАПРАВЛЕНИЙ.
	//
	// Сам релей материализуется ВСЕГДА — он часть маршрута, и без него узел
	// пошёл бы напрямую, то есть в блокировку (ровно это и происходило, пока
	// хопы отбрасывались на материализации). Галка про другое: релей это
	// дозвонщик, а не «страна», и в списке целей Направления ему по умолчанию
	// не место. Кому нужно — включает и выбирает его как обычный узел.
	//
	// В составе подписки и в хопах цепочки релей виден при любом значении
