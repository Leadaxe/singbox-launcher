...
type Source struct {
	...
	// ExposeRelays — показывать ли служебные узлы подписки (релеи BYPASS)
	// отдельными записями состава.
	//
	// Выключено (умолчание): релей живёт ВНУТРИ тела своего узла и человеку
	// невидим — его нельзя ни выключить, ни выбрать, а каждый fetch собирает
	// его заново. Включено: релей становится самостоятельным узлом с меткой
	// Service, у него своя галка и свой адрес для ссылок, а владелец
	// дозванивается через него полем Detour.
	ExposeRelays bool `json:"expose_relays,omitempty"`
