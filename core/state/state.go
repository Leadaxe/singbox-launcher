...
type State struct {
	...
	// Channels — каналы роутинга (SPEC 104): именованные точки выбора, на
	// которые ссылаются правила.
	//
	// nil означает «секции ещё нет»: при первой загрузке она засевается из
	// `default_channels` шаблона. Пустой непустой слайс — это осознанный
	// выбор пользователя (все каналы удалены), и повторно засевать его
	// нельзя, иначе удалённое возвращалось бы на каждом старте.
	Channels []Channel
