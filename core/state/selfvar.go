...
//
// Обходчик шаблона (`template.SubstituteVarsInJSONCanon`) считает любое
// `@имя` ссылкой на переменную: необъявленное — опечаткой с предупреждением,
// объявленное без значения — поводом выбросить ключ. У секции узла словаря
// нет вовсе, а строки её тел пишет пользователь: домен `@example`, путь,
// комментарий — всё это превратилось бы в предупреждения и выпавшие ключи. Здесь правило другое и
