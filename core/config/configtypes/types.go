...
type ProxySource struct {
	...
	// Chains — цепочки источника, СОБРАННЫЕ НА ЛЕТУ проходом 2
	// (config.ResolveCanonicalChainHops): по записи на каждый включённый
	// узел kind=chain — настройки маршрута из тела узла плюс позиции с уже
	// разрешёнными финальными тегами.
	//
	// Список, а не одно поле: у корневой цепочки запись одна, у папки — по
	// записи на каждую цепочку её состава. Одно поле на источник склеивало
	// две цепочки папки в один outbound — тег первой, позиции последней, а
	// вторая пропадала без предупреждения (NODE_LINK.md §9.3 п. 2).
	//
...
// BuiltChain — одна цепочка источника в сборочной форме: тег её узла и
// маршрут с разрешёнными позициями (ProxySource.Chains, дериват прохода 2).
type BuiltChain struct {
	// Tag — сырой тег узла-цепочки, он же финальный: тег-политика контейнера
	// к цепочке не применяется. Пусто — сборка даст запасное `chain-<N>`.
	Tag string
