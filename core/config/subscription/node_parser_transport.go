...
// D-104).
//
// Явный отпечаток узла уходит в конфиг как есть. D-104 подменял всё вне
// chrome-семейства на chrome, исходя из того, что КАЖДЫЙ REALITY-сервер —
// Xray ≥ v26.9.8, которому нужен key_share X25519MLKEM768. Поле показало
// обратное: старые серверы принимают firefox без MLKEM, а chrome с гибридным
// ключом даёт ClientHello больше одного TCP-сегмента, который в мобильных сетях
// режет DPI. Провайдер, поставивший firefox, делает это сознательно, и выбор
// остаётся за ним — как у клиента Xray.
//
// Что правится:
