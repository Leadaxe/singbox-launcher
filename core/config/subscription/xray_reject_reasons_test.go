...
// Протокол взят trojan-go: ни одна секция его не ведёт, и в набор СЛУЖЕБНЫХ
// (freedom/blackhole/dns — законная часть конфига, молча не становятся узлами)
// он не входит, то есть это ровно класс «протокол не поддержан». Прежде здесь
// стоял wireguard — с контракта 1.1.50 у него есть mappers.xray, и элемент
// перестал быть «неподдержанным»: он стал разбираемым и, без secretKey,
// законно отбраковывается с причиной. Подставить сюда схему, которую мы как
// раз научились читать, значило бы проверять обратное тому, что заявлено.
func TestXrayUnsupportedProtocolStaysSeparate(t *testing.T) {
	raw := `[{
	  ...
	  "outbounds": [{
	    "protocol": "trojan-go",
	    "tag": "proxy",
	    "settings": { "servers": [{ "address": "t.test", "port": 443 }] }
	  }]
	}]`
