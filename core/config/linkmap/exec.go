...
// Коды ОТКАЗА разбора (CANON §4, D-088, контракт 1.1.48).
//
...
func (st *execState) applyEntry(e *Entry) {
	...
	if code := codeOf(p.OnPresent); code != "" {
		...
		// Без него две таких записи на одном узле схлопывались бы в один
		// код (дедуп идёт по паре «код, path»), и вторая потеря осталась
		// бы молчаливой.
