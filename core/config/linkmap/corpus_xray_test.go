...
func TestEngineVsXrayCorpus(t *testing.T) {
	...
	// Ноль СВЕРЕННЫХ кейсов — отказ. Файлы на диске ещё не означают сверку:
	// каждый кейс умеет пропустить себя сам (не один узел, не один элемент),
	// и корпус, весь ушедший в такие пропуски, зеленел бы молча.
	if checked == 0 {
		t.Fatal("ни один кейс xray не сверен — все пропущены")
	}
