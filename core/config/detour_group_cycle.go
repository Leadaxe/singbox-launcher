...
//
// Лечение то же, что у прочих дефектов detour (SPEC 077): fail-open —
// узел выпадает из СОСТАВА группы, но остаётся в конфиге и продолжает
// ходить своим детуром. Снять detour было бы хуже: пользователь задал его
// осознанно, и тихо отправить трафик напрямую значит нарушить ровно то, о
// чём он просил.
package config
