...
// Переходный писатель формата 0.12 (SPEC 127 §4, окно совместимости).
//
// Пока релиз LxBox с чтением 1.0 не доехал до пользователей, лаунчер обязан
// уметь писать прежний файл: файлы гуляют между десктопом и телефоном у живых
// людей, и односторонний переход оставил бы их без импорта на дни. Поэтому
// экспорт держит ДВУХ писателей, и этот — прежний, маппер «состояние → форма
// 0.12». Дефолт переключается одной константой BackupExportFormatDefault
// (export.go), после релиза LxBox этот файл удаляется целиком.
//
// Инвариант: файл 0.12 из Export012 — БАЙТ-В-БАЙТ такой же, каким его писал
// сегодняшний экспорт. Эталоны — testdata/export012_*.json, сверка —
// export012_etalon_test.go; менять здесь что-либо «заодно» запрещено.
//
// ЕДИНСТВЕННОЕ отличие от прежнего вывода — секции узлов (SPEC 121 §10):
// норма ONE_NAMESPACE §4 везёт их только в 1.0, потому что 0.12 объявляет их
// схемой одной формы, а лаунчер писал туда форму состояния (ловушка CODEMAP
// §7.6 — код и схема расходились, и этого никто не ловил). Здесь они больше
// не пишутся, а потеря называется вслух (backup_local_only_dropped, поле
// `sections`). Читаются файлы с секциями по-прежнему: старые файлы уже у
// пользователей на руках.
...
func Export012(s *state.State, opts ExportOptions) (*Backup, []Warning, error) {
	if s == nil {
	...
	for i, src := range s.Sources {
		...
		case state.SourceKindServer:
			...
			// Секции узла в 0.12 не едут (см. шапку файла): молчаливая потеря
			// здесь была бы самой дорогой — узел приедет на новую машину без
			// своих правил и DNS, и пользователь увидит это не раньше, чем
			// перестанет работать маршрут, ради которого секции заводились.
			if !src.Sections.IsEmpty() {
				warnings = append(warnings, Warning{
					Code:   WarnBackupLocalOnlyDropped,
					Detail: sourceExportName(src) + ": sections",
				})
			}
...
func exportSourceIdentity(src state.Source) *SubscriptionIdentity {
	...
	if ua := strings.TrimSpace(src.IdentityUserAgent()); ua != "" {
		out.UserAgent = &ua
	}
	if hwid := strings.TrimSpace(src.IdentityHWID()); hwid != "" {
		out.HWID = &hwid
	}
	if v := src.IdentitySendHWID(); v != nil {
		b := *v
		out.SendHWID = &b
	}
	if v := src.IdentityHashDeviceModel(); v != nil {
		b := *v
		out.HashDeviceModel = &b
	}
	if out.IsEmpty() {
		return nil
	}
...
func exportFolder(src state.Source) ([]Server, string) {
	...
	for _, n := range src.Nodes {
		...
		if !n.Sections.IsEmpty() {
			// Как и у корневого узла: секции формат 0.12 не выражает.
			lost = append(lost, "sections "+n.Tag)
		}
