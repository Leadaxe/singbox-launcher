...
func decode10(b *Backup10, opts ImportOptions) (*decodedFile, error) {
	...
	// для одного: тега замены свёрнутой подписки. Формат 1.0 его не несёт
	// (`fold` — форма контракта, а там тега нет: в 0.11 он был позиционным
	// деривативом, D-081), и экспорт 1.0 считает тот же дериватив, чтобы
	// сказать о расхождении (WarnBackupReplaceTagDerived). Значит импорт
	// обязан воспроизвести ровно ту же формулу, иначе правила ЭТОГО ЖЕ файла
	// уехали бы в никуда, а круг «экспорт → импорт → экспорт» разошёлся бы.
	subIndex := 0
