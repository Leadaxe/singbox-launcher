...
// (SPEC 121 §3.2).
//
// Фрагменты здесь СЫРЫЕ, до подстановки `@self` и префиксов: разворачивание
// живёт в node_sections_expand.go и зависит от финального тега, который
// известен только здесь.
type NodeSectionSet struct {
	// FinalTag — тег, под которым узел эмитится в конфиг (после тег-политики
	// контейнера). Значение `@self` и префикс тегов DNS-серверов.
	FinalTag string
	// Link — идентичность узла в состоянии: по ней якорь kind=node находит
	// свой набор секций.
	Link NodeLink
	// DNSServers / DNSRules / Rules — тела фрагментов как их написал
	// пользователь (порядок ключей значим).
